...
Miget/provider-owner ACL baselines can leave a public `SECURITY INVOKER` RPC executable by `service_role` while stripping EXECUTE from a nested private helper. The normal scheduler then fires successfully but the RPC fails before leasing a row. Every NEW/UPGRADE release must run role-specific private-helper verifiers after final repeatables; the email outbox requires `23082026_0822_candidate_manager_email_claim_route_guard_verification.sql`. Do not broaden the repair to `anon` or `authenticated`, and prove both the public RPC and each required private helper through the effective `service_role` before declaring a drain healthy.
