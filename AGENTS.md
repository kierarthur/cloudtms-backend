...
The immutable Supabase migration `26072026_1219_disable_runtime_plpgsql_check.sql` hard-codes database `postgres`. Miget agency services use generated database names, so LEGACY_UPGRADE must replace that migration with `supabase/release/30082026_0958_legacy_plpgsql_check_database_defaults_replacement.sql`. It applies and catalogue-verifies the same seven passive-instrumentation defaults against `current_database()` only. Never rewrite the immutable migration, target another database by name, change extension membership, or treat a provider database-name mismatch as permission to skip the intended runtime protection.
