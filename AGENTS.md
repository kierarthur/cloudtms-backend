...
The Candidate-named browser-isolation replacement has two and only two accepted service ACL prestates: the sealed clean LEGACY profile and the separately sealed interrupted profile produced when an earlier fail-closed attempt installed five repository-authorised service grants before managed adoption. Both share the exact routine-identity manifest, preserve their exact service matrix, and converge to zero browser execution. Never loosen these fingerprints, grant missing service execution merely to match one profile, or accept a third ACL shape by count alone.
