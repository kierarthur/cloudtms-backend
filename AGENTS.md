...
Any Cloudflare Worker that reaches a public `*.workers.dev` Miget compatibility gateway must retain the `global_fetch_strictly_public` compatibility flag. Without it, Worker health and readiness can pass while real gateway requests and scheduled jobs fail with Cloudflare error `1042`. Every NEW/UPGRADE runtime acceptance must therefore prove both a harmless gateway-backed database read and every expected cron on the final active Worker version; health-only evidence is insufficient. The legacy LIVE `arthur-rai-broker` additionally requires its five-minute Sheets outbox retry to complete without exceptions.
...
Miget PostgreSQL does not supply Supabase's `auth` schema. `supabase/release/30082026_0030_miget_auth_compatibility_bootstrap.sql` is therefore the first LEGACY_UPGRADE bootstrap and the first NEW baseline file. It creates only the provider-neutral one-column `auth.users` foreign-key target plus `auth.uid()`, `auth.role()` and `auth.jwt()` claim helpers; it copies no user data and does not replace `public.tms_users` as CloudTMS authentication authority. Never omit, reorder or hand-recreate this bootstrap for a Miget LEGACY_UPGRADE/NEW release.

Historical LIVE workbench state predates `ux_bpay_workbench_sessions_shared_context_open`, while migration `22062026_1025_workbench_legacy.sql` correctly refuses to drop the actor-scoped index until that replacement exists. LEGACY_UPGRADE must run `supabase/release/30082026_0055_legacy_workbench_shared_context_bootstrap.sql` after the legacy Banking table bootstrap and before repository migrations. It proves all five required columns, rejects any unexpected same-named index or duplicate open shared-context group, and creates only the exact replacement unique index; it must never delete or merge workbench data. NEW already receives the same index from its structural baseline and must not use this legacy-only bootstrap as a substitute.

Historical or interrupted LIVE may retain the current pay-summary trigger while lacking `public._pay_batch_status_is_active_reservation(text)`. Because `supabase/repeatable/08042026_1151_newtablesbanking.sql` updates `pay_batches` and fires that trigger, LEGACY_UPGRADE must run `supabase/release/30082026_0321_legacy_pay_batch_status_helper_preload.sql` before that Banking bootstrap. A later historical migration can replace/remove the helper, so run the same exact idempotent file again as the final repeatable preload before convergence. The preload installs only the signed helper and its service-only ACL; it changes no payment state, financial rule or Policy X authority. Preserve an executable regression and a missing-helper disposable rehearsal for both positions.

The same early Banking owner can fire current triggers that call `public._temp_diag_log(text,text,text,jsonb)` after the historical migration chain has removed that helper. LEGACY_UPGRADE must run `supabase/release/30082026_1038_legacy_temp_diag_log_preload.sql` before `08042026_1151_newtablesbanking.sql` and again in the repeatable-preload phase. It must remain the exact current function body and service-only ACL, must not enable `settings_defaults.temp_log`, and must not call the function or write an audit row during preload.

Every LEGACY general/Candidate routine and ACL prestate fingerprint must be regenerated from a fresh, ACL-preserving restore of the exact hosted checkpoint after the complete current preload and repeatable ordering has run. The general browser-isolation inventory must include the service-only `_temp_diag_log` preload; post-repeatable convergence must include that same identity and the five repository-authorised Candidate service grants already installed by the current ordering. Routine identities must use explicit function/type catalogue schemas and names, never `regprocedure` display text. Never derive a durable fingerprint from a disposable database that has already crossed a later failed post-repeatable step, and never relax a count/hash to admit an unexplained third state.

The immutable Supabase migration `26072026_1219_disable_runtime_plpgsql_check.sql` hard-codes database `postgres`. Miget agency services use generated database names, so LEGACY_UPGRADE must replace that migration with `supabase/release/30082026_0958_legacy_plpgsql_check_database_defaults_replacement.sql`. When the provider exposes the seven settings, apply and catalogue-verify them against `current_database()` only. Miget currently exposes neither the `plpgsql_check` extension nor any `plpgsql_check.*` setting and rejects those superuser-only database GUCs; in that exact case, the replacement may treat the migration as non-applicable only after proving all three facts: extension absent, runtime-setting catalogue empty, and no persisted database-wide `plpgsql_check.*` override. Any partial/mixed state still fails closed. Never rewrite the immutable migration, target another database by name, change extension membership, catch and ignore `insufficient_privilege`, or infer non-applicability from a database-name mismatch alone.

The provider owner mapper may omit only the seven exact `plpgsql_check` diagnostic function settings when the provider proves the exact unavailable state; it must preserve every function body, owner/grant statement, `search_path`, timeout and unrelated setting. A multiline `ALTER FUNCTION ... SET plpgsql_check.*` matcher must never cross a semicolon. The Banking Pay catalogue pre-apply generator must execute provider-mapped include files in the repository release order rather than original source paths, and a non-superuser disposable replay must prove that the mapped catalogue rehearsal leaves surrounding definitions intact before hosted APPLY.

Generated PostgreSQL verification SQL must treat `OVERLAY` as a SQL syntax construct and must never emit `pg_catalog.overlay(...)`; the schema-qualified form is rejected at execution time even when source generation and ordinary JavaScript tests pass. Keep an executable generator assertion for both the permitted `overlay(...)` form and the forbidden prefix.

LEGACY_UPGRADE data-preservation proof must distinguish preserved customer rows from repository-authorised additive configuration seeds. Migration `26082026_2057_candidate_system_actor_seed.sql` may add exactly one inactive, non-login, non-payment MyTMS Candidate system actor only when `settings_defaults.candidate_app_system_actor_user_id` is null; it never replaces an existing configured actor. The manager-email migrations may add their exact versioned default-template rows. Record those expected additions separately, and still prove that existing Client, Candidate, Timesheet, Contract, invoice and payment rows were neither deleted nor rewritten by the upgrade.

A non-superuser Miget database owner cannot bypass `FORCE ROW LEVEL SECURITY`, even when it owns the table. Never accept a superuser-only local rehearsal as proof that a later migration can write a table after an earlier migration forced RLS without an applicable policy. Prefer completing the bounded write before forcing RLS. When immutable published order prevents that in LEGACY_UPGRADE, use a repository-controlled atomic replacement that verifies the exact owner/table/RLS/policy shape, temporarily removes only `FORCE`, performs the exact owner write, restores `FORCE`, and verifies the final state; any error must roll back the temporary relaxation. Rehearse that path as a real non-superuser owner and require final RLS, policy, owner, ACL and contract verification before application deployment.

LEGACY_UPGRADE is resumable after a fail-closed hosted stop. Its structural bootstrap can have created `public.schema_repeatables` before managed adoption. A resumed plan may accept that table only when it has exactly `filename text`, `content_sha256 text`, and defaulted `applied_at timestamptz`, the exact `filename` primary key, and zero rows. Any populated ledger, unexpected column/default/type, missing primary key, or managed identity remains an ambiguity and must fail closed. Never delete or clear a ledger to make this gate pass.

A LEGACY_UPGRADE APPLY is fail-closed but is not one transaction across the whole release: each completed migration and its public ledger row can commit before a later repeatable/verifier stops. After any hosted failure, inspect the fresh hosted migration/repeatable/identity state and rerun PLAN; never reason from the original plan alone. Before another hosted APPLY, complete both (a) a full data-bearing PostgreSQL 17 rehearsal restored from the preserved encrypted pre-upgrade checkpoint under a non-superuser provider owner and (b) a fresh schema/ACL-only clone of the exact interrupted hosted checkpoint. A Miget owner cannot dump rows protected by `FORCE ROW LEVEL SECURITY`; never weaken RLS to obtain a rehearsal dump. Use the already preserved encrypted checkpoint for customer-data proof and copy only non-customer release metadata into a schema-only clone.

The Candidate-named browser-isolation replacement accepts one exact provider-neutral current prestate: 85 routine identities, eight service-role omissions and the sealed service/ACL hashes produced by the current repository ordering. Both the preserved clean pre-upgrade rehearsal and the exact hosted interrupted checkpoint must converge to that same canonical prestate before browser execution is removed. Historical `regprocedure` display hashes are not accepted because they vary with `search_path`; never add a second display-only hash, grant missing service execution merely to match a profile, or accept an unexplained ACL shape by count alone.

The original LIVE ledger also omits historical function deployments that later migrations attach as triggers or inspect for ownership. `supabase/release/24082026_1128_legacy_upgrade_trigger_shims.sql` is the bounded LEGACY_UPGRADE bridge for that historical state: it creates only the explicitly inventoried temporary trigger/RPC signatures and the exact archive capability table, revokes browser/service execution of callable shims, and marks every temporary function with `CLOUDTMS_LEGACY_TRANSITION_SHIM`. Current repeatables must replace every marked definition before verification or atomic adoption; `assertLegacyTransitionShimsReplaced()` must inventory the complete shim list and fail closed if one remains. Never use these shims for NEW or managed UPGRADE, never treat them as durable business implementations, and never remove the final replacement assertion.
...
The connector's `MIGET_API_TOKEN` must be a dedicated Miget workspace token set to `Expires = Never`, `All projects`, and the read-only preset. It belongs only in the Cloudflare Worker secret and must never be printed or committed. If the API-backed parity/infrastructure/service tools fail while Hyperdrive-backed database tools still work, inspect token expiry before diagnosing Miget or the databases; replace only the connector secret with another least-privilege read-only token and rerun the full nine-tool preflight. Do not give this audit connector manage, operate or deploy permissions.
...
PostgreSQL composite-row assignment regression (30 August 2026): when a PL/pgSQL target is declared with `%ROWTYPE`, select the complete relation shape explicitly with `SELECT alias.* INTO row_variable`. `SELECT alias INTO row_variable` can compile but is treated as one composite value and may fail on first use by attempting to cast the whole textual row into the target's first scalar column. Keep the rollback-contained `30082026_0455_candidate_daily_emergency_window_row_verification.sql` mandatory in NEW and UPGRADE; it must execute both the private booked-shift helper and the public emergency-window reader with normal ACLs, and reject the unsafe source form. Compilation, a function hash, or an empty-window call does not prove this boundary.
...
Google Rota reconciliation scalability regression (29 August 2026): a full Availability refresh must inspect the complete Candidate list, not stop at a convenient first page, but it must not post fourteen dates for every non-enrolled Candidate. Probe each Candidate identity once through the signed reconciliation route, accept only the server-owned `LINKED` result as authority to perform the full 14-day check, and treat only the server-owned `NOT_ENROLLED` classification as a benign skip. Ambiguous, invalid and unknown results remain terminal failures. The Apps Script request cap is a checkpoint unit, not a per-trigger population cap: while cooperative time remains, a continuation may complete multiple bounded requests, saving every accepted cursor before the next request, and a later trigger resumes from the next unacknowledged item. Do not persist contact data or source HMACs in continuation state. In the database, an identity probe is read-only; normal date reconciliation applies every linked date first and refreshes the Candidate sync state exactly once per linked Candidate, never once per date. Keep `29082026_0923_candidate_daily_reconciliation_linked_window_verification.sql`, `29082026_1407_candidate_daily_reconciliation_identity_probe_verification.sql` and `29082026_1408_candidate_daily_reconciliation_identity_probe_runtime_verification.sql` mandatory in UPGRADE and NEW, and retain interruption/resume coverage proving every Candidate row is probed, acknowledged cursors are not replayed, an all-linked population completes without duplicate work, and identity probes create no availability/projection/sync/command state.

Candidate rejection runtime-fixture regression (29 August 2026): a focused database workflow must install replacement repeatables in their real successor order, because a later base file can silently overwrite a correction while every individual SQL file still compiles. In `.github/workflows/candidate-db-runtime.yml`, keep `26082026_0725_candidate_authorised_hours_expense_anchor_v1.sql` after its `07082026_2108` base, keep `27082026_0423_candidate_electronic_rejection_resubmission_v1.sql` after its `07082026_2128` base, install the Daily booked-source and receipt-context private helpers plus the weekly/Daily withdrawal-reset owner and refused-card recovery successor before the final transition authority, and keep the current expenses-only resubmission correction last. A Daily runtime fixture that crosses an entitlement boundary must seed the explicit enabled entitlement while preserving any intended negative proof through its missing or removed Candidate identity. Weekly withdrawal fixtures must carry a valid configured system actor and must prove that withdrawal reopens the Contract Week without manufacturing a replacement Timesheet; Daily withdrawal retains its separate versioned same-shift record because Daily has no Contract Week. An expenses-only rejected claim must remain anchored to the original worked Timesheet, never the newly rotated empty expense carrier. Retain rollback-contained first-use coverage for hours-only, combined hours and expenses, separate expense claims, durable single resubmission lineage and withdrawal/reset paths; compilation, static source checks or one route alone are not sufficient.
...
A queued Candidate source-build row whose run identity or enabled authority-fingerprint contract cannot pass the current claim is terminal compatibility work, not a payload that may be patched or guessed into validity. Before any two-call source-build claim, the service-only repair must take the established Candidate serial, Candidate, registry, session, scope and exact job locks; terminalise every proved-invalid queued row for that same open session/Candidate as `DEAD`; preserve any already-valid current owner; and delegate replacement only to `pay_workbench_repair_orphaned_pending_source_build` / `pay_workbench_enqueue_candidate_refresh`. It must then prove that no current scope or Candidate-state pointer references a terminal row, that the canonical successor carries the current session/version/change sequence and required fingerprint/publication contract, and that no invalid active source owner remains. `DEAD` rows remain terminal audit evidence and are excluded from claim and active-dedupe indexes; do not delete or recycle them in the compatibility repair. Keep `29082026_1313_banking_pay_invalid_source_authority_repair_verification.sql` mandatory in NEW and UPGRADE. It must reproduce the pre-repair `22023` claim failure, cover multiple invalid rows for one Candidate, preserve an existing canonical owner, prove idempotency and service-only ACL, and prove no Draft, batch, provider, payment, settlement or remittance action.
