...
For every Miget CloudTMS/MyTMS PostgREST app, append `options=-c%20pg_show_plans.is_enabled%3Doff%20-c%20statement_timeout%3D120s` to `PGRST_DB_URI`, preserving all other URI components and credentials. Miget currently preloads `pg_show_plans`; leaving it enabled caused repeated `not enough memory to append new query plans` warnings and multi-second complex-RPC latency. The connection-level `statement_timeout` is also mandatory: a function-level `SET statement_timeout` does not prove that the already-running top-level PostgREST statement is bounded. Redeploy and prove a new PostgREST session has the collector disabled and the 120-second timeout, the warning flood is absent, and an exact real RPC passes a timing benchmark. After any resource resize, independently verify the live limits plus PostgreSQL memory settings instead of trusting the control-plane allocation alone.
...
The established Git-connected synthetic Candidate build invokes the canonical `candidate-synthetic-private-api/wrangler.jsonc`. The historical `candidate-private-api/wrangler.synthetic.jsonc` alias must not be restored. The Git build must use `npx wrangler deploy --dry-run --config candidate-synthetic-private-api/wrangler.jsonc` and deploy with `npx wrangler deploy --config candidate-synthetic-private-api/wrangler.jsonc --keep-vars`; `tests/candidate-broker-boundary.test.js` enforces the canonical path. A successful local deployment does not excuse a failing Git trigger.
...
Federated Candidate session epochs are monotonic only inside one global control-plane session family. Different devices and independent re-logins may legitimately have unrelated epoch sequences and must remain concurrently usable; never compare or revoke one family's epoch because another family has a larger number. Carry the control plane's family identity through sealed access/refresh and signed private route contexts, hash it at the agency boundary, and project it with the service-only family-aware owner. Account, membership, Candidate-access and explicit global revocation remain cross-family security boundaries. Preserve ordered rollout compatibility for already-issued family-less tokens, retire their legacy projection on the first family-aware request, and require rollback-contained PostgreSQL proof of two concurrent families, same-family renewal, stale replay rejection, conflicting replay rejection and browser-denied execution. When diagnosing a real login failure, capture only the closed error code and request identifier at the private boundary; do not infer network or credential failure from the public generic message and never log tokens or sensitive payloads.

Candidate manager finalisation must never consume the shared PostgREST connection pool. Every PHONE/EMAIL finalisation and replay probe must call the service-only non-waiting single-flight wrapper, which holds one global transaction-scoped advisory gate plus the exact workflow/generation gate and returns a safe pending receipt immediately when either is busy. Worker RPC calls are bounded to 20 seconds, recovery remains sequential with a retained per-workflow lease, and every PostgREST database session is independently bounded by the mandatory 120-second connection-level statement timeout. Never restore a direct Worker call to the raw atomic finaliser. Acceptance must deliberately hold the finalisation gate, prove the competing call defers promptly, and prove unrelated login/read traffic remains healthy; a safe pending claim is preferable to making the agency unavailable.
...
An incrementally changed recursive repeatable must also finish in its own approved ACL state. If a root `\ir`-includes a historical function page whose original grants are now superseded, that same recursive closure must include the current ACL reassertion after every definition that can restore the old grant. Do not rely on a separate unchanged repeatable being selected again: UPGRADE applies only changed closures. Keep a source-order regression for that dependency and require the final browser-isolation verifier to pass after the exact changed-closure replay.
...
The first-choice contract step is to reuse the same already-running clean PostgreSQL 17 proof database and run `npm run db:contract:seal-local` before the first push. This local-only command verifies PostgreSQL 17, exports `supabase/release/current-contract.json` in seconds, and avoids a second rebuild. Commit the reviewed database source and generated contract together. `npm run db:contract:coupling` and both source gates must reject a contract-bearing migration or repeatable when that exact commit omits the generated contract. Never use a hosted APPLY attempt to discover that the fingerprint is stale.

Before starting a disposable local NEW/UPGRADE proof, preflight the complete release profile once: the fixture database must be named exactly `banking_modal_v2_test`, local execution must set `CLOUDTMS_ALLOW_LOCAL=1`, and provider-parity execution must set `CLOUDTMS_LOGICAL_POSTGRES_OWNER=CURRENT_USER`. Fail before applying SQL when any prerequisite is absent. Do not spend a full replay discovering a wrong database name or owner mapping in a late verifier.
