-- TEST rollback captured immediately before the financial-position-only Phase 3 carrier fallback.
-- Restores only the two exact source-specific three-argument Phase 3 definitions.
BEGIN;
...
REVOKE ALL ON FUNCTION public.hr_weekly_phase3_apply_adjustment_truth(uuid,text[],uuid) FROM PUBLIC, anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.nhsp_weekly_phase3_apply_adjustment_truth(uuid,text[],uuid) FROM PUBLIC, anon, authenticated, service_role;
