...
test('current Invoice V8 headers may prove VAT from one consistent line rate', () => {
  assert.match(
    body,
    /when nullif\(btrim\(coalesce\(i\.header_snapshot_json->>'applied_vat_rate_pct',''\)\),'\'\) is null\s+then false/,
  );
  assert.match(
    body,
    /if v_invoice_rate_count > 1 then\s+raise exception 'CORRECTION_POLICY_ROOT_INVOICE_VAT_AMBIGUOUS'/,
  );
  assert.match(
    body,
    /if v_invoice_line_count > 0 and v_invoice_chargeability_count <> 1 then\s+raise exception 'CORRECTION_POLICY_ROOT_INVOICE_CHARGEABILITY_UNPROVEN'/,
  );
  assert.match(
    body,
    /lower\(i\.header_snapshot_json->>'vat_chargeable'\)='false'\s+and il\.vat_rate_pct<>0/,
  );
});
