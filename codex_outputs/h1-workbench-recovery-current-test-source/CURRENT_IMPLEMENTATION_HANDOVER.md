...
## 11. Backend test portability successor

The backend causal-recovery test uses `tests/fixtures/banking-pay-workbench-f013-bible-evidence-v1.json` for the narrow append-only F-013 claim-ordering assertion. The fixture carries the exact external candidate identity and exact asserted marker strings in source order, and is self-contained and reviewable in this repository. It is not the canonical frontend `BANKING_PAY_BIBLE.md`, does not replace or publish that document, and grants no Banking runtime or policy authority. The canonical frontend Bible remains external and unpublished by this backend-only portability successor. A reviewer may supply `BANKING_PAY_BIBLE_PATH` to make the same test hash and read the real candidate, but ordinary npm and CI runs do not depend on an external worktree or environment variable.
