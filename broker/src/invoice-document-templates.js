...
export function renderInvoiceReferencePanel(model = {}) {
  ...
  const workers = [...new Set((Array.isArray(model.lines) ? model.lines : [])
    ...
    .filter(Boolean))];
  const candidateSummary = model.suppress_candidate_header === true
    ? ''
    : workers.length === 1
    ? workers[0]
    : workers.length === 0
...
export function renderInvoiceLineTable(model = {}) {
  ...
  const weekEndingLayout = model.template_version === 'invoice-professional-v2'
    || rows.some(line => Object.hasOwn(line || {}, 'week_ending_date'));
  const colgroup = weekEndingLayout
    ? '<col style="width:15%"><col style="width:14%"><col style="width:11%"><col style="width:14%"><col style="width:7%"><col style="width:12%"><col style="width:9%"><col style="width:8%"><col style="width:10%">'
    : '<col style="width:15%"><col style="width:13%"><col style="width:13%"><col style="width:8%"><col style="width:8%"><col style="width:12%"><col style="width:10%"><col style="width:10%"><col style="width:11%">';
  const header = weekEndingLayout
    ? '<th class="item-type">Rate / item type</th><th>Worker</th><th>Week ending</th><th>Reference</th><th class="number">Units</th><th class="number">Cost per unit</th><th class="number">Net</th><th class="number">VAT</th><th class="number">Total</th>'
    : '<th class="item-type">Rate / item type</th><th>Worker</th><th>Reference</th><th>Unit</th><th class="number">Units</th><th class="number">Cost per unit</th><th class="number">Net</th><th class="number">VAT</th><th class="number">Total</th>';
  return `<table class="invoice-lines"><colgroup>${colgroup}</colgroup><thead><tr>${header}</tr></thead><tbody>${rows.map(line=>`<tr><td class="item-type">${escapeInvoiceHtml(line.description)}</td><td>${escapeInvoiceHtml(line.worker)}</td>${weekEndingLayout ? `<td class="week-ending">${escapeInvoiceHtml(formatFrozenDocumentValue(line.week_ending_date,{kind:'date'}))}</td>` : ''}<td>${escapeInvoiceHtml(line.reference)}</td>${weekEndingLayout ? '' : `<td>${escapeInvoiceHtml(line.unit)}</td>`}<td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.quantity,{kind:'number',maximumFractionDigits:4}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.unit_price,{kind:'money',currency}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.net_amount,{kind:'money',currency}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.vat_amount,{kind:'money',currency}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.gross_amount,{kind:'money',currency}))}</td></tr>`).join('')}</tbody></table>`;
}
