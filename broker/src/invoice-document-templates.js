...
export function buildNhspSupportHtml(model={}) { const m=validateFrozenPresentationModel('NHSP_SUPPORT',model); return supportTable('NHSP support',m,[['worker','Worker'],['nhsp_shift_id','Assignment / shift ID'],['booking_reference','Booking / reference'],['site_ward','Site / ward'],['shift_date','Date','date'],['shift_times','Times'],['hours_units','Hours / units'],['source_identity','Source'],['validation_state','Validation']].map(([key,label,kind])=>({key,label,kind}))); }
