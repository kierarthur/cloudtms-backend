...
export function renderInvoiceLineTable(model = {}) {
  ...
  return `<table class="invoice-lines"><colgroup><col style="width:15%"><col style="width:13%"><col style="width:13%"><col style="width:8%"><col style="width:8%"><col style="width:12%"><col style="width:10%"><col style="width:10%"><col style="width:11%"></colgroup><thead><tr><th class="item-type">Rate / item type</th><th>Worker</th><th>Reference</th><th>Unit</th><th class="number">Units</th><th class="number">Cost per unit</th><th class="number">Net</th><th class="number">VAT</th><th class="number">Total</th></tr></thead><tbody>${rows.map(line=>`<tr><td class="item-type">${escapeInvoiceHtml(line.description)}</td><td>${escapeInvoiceHtml(line.worker)}</td><td>${escapeInvoiceHtml(line.reference)}</td><td>${escapeInvoiceHtml(line.unit)}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.quantity,{kind:'number',maximumFractionDigits:4}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.unit_price,{kind:'money',currency}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.net_amount,{kind:'money',currency}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.vat_amount,{kind:'money',currency}))}</td><td class="number">${escapeInvoiceHtml(formatFrozenDocumentValue(line.gross_amount,{kind:'money',currency}))}</td></tr>`).join('')}</tbody></table>`;
}
