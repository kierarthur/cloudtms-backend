...
// ─────────────────────────────────────────────────────────────────────────────
// Outbox: mark a queued item as SENT (callback from provider/flow)
// ─────────────────────────────────────────────────────────────────────────────
async function handleOutboxMarkSent(env, req) {
  ...
  const body = await parseJSONBody(req);
  if (!body || !body.id) return withCORS(env, req, badRequest("id is required"));

  const sentAt = body.sent_at || new Date().toISOString();

  try {
    const res = await fetch(
      `${env.SUPABASE_URL}/rest/v1/mail_outbox?id=eq.${encodeURIComponent(body.id)}`,
      {
        method: 'PATCH',
        headers: sbHeaders(env),
        body: JSON.stringify({
    ...
    if (!res.ok) {
      const err = await res.text();
      return withCORS(env, req, serverError(`Outbox mark-sent failed: ${err}`));
    }

    await writeAudit(env, user, 'MAIL_OUTBOX_MARK_SENT', {
      id: body.id,
      provider_message_id: body.provider_message_id || null
    });
    ...
    return withCORS(env, req, serverError("Failed to mark sent"));
  }
}

// ─────────────────────────────────────────────────────────────────────────────
// Outbox: mark a queued item as FAILED (callback from provider/flow)
// ─────────────────────────────────────────────────────────────────────────────
async function handleOutboxMarkFailed(env, req) {
  ...
  const body = await parseJSONBody(req);
  if (!body || !body.id || !body.error) {
    return withCORS(env, req, badRequest("id and error are required"));
  }
  ...
  try {
    const res = await fetch(
      `${env.SUPABASE_URL}/rest/v1/mail_outbox?id=eq.${encodeURIComponent(body.id)}`,
      {
        ...
        body: JSON.stringify({
          status: 'FAILED',
          failed_at: failedAt,
          last_error: String(body.error).slice(0, 2000)
        })
      }
    );
    ...
    if (!res.ok) {
      const err = await res.text();
      return withCORS(env, req, serverError(`Outbox mark-failed failed: ${err}`));
    }

    await writeAudit(env, user, 'MAIL_OUTBOX_MARK_FAILED', {
      id: body.id,
      error: body.error
    });
    ...
    return withCORS(env, req, serverError("Failed to mark failed"));
  }
}
...
async function handleRemittanceEmailForCandidate(env, req) {
  ...
  try {
    // ----------------------------
    // Fetch financial rows (current snapshot) + TS & Client context
    // ----------------------------
    let url = `${env.SUPABASE_URL}/rest/v1/timesheets_financials` +
              `?select=` +
              [
    ...
    if (timesheetIds.length) {
      const ids = timesheetIds.map(id => encodeURIComponent(id)).join(',');
      url += `&timesheet_id=in.(${ids})`;
    } else {
      url += `&candidate_id=eq.${encodeURIComponent(candidateId)}`;
      url += `&timesheet.week_ending_date=gte.${encodeURIComponent(startDate)}`;
      url += `&timesheet.week_ending_date=lte.${encodeURIComponent(endDate)}`;
    }

    // Sort by week ending ascending for readability
    url += `&order=timesheet.week_ending_date.asc`;

    const { rows: finRows } = await sbFetch(env, url, false);
    if (!finRows.length) {
      return withCORS(env, req, notFound("No timesheets found for the selection"));
    }

    // Ensure all rows belong to a single candidate (one email per candidate)
    const candIds = [...new Set(finRows.map(r => r.candidate_id).filter(Boolean))];
    if (candIds.length !== 1) {
      return withCORS(env, req, badRequest("Selection spans multiple candidates; send one remittance per candidate."));
    }
    const candId = candIds[0];

    // ----------------------------
    // Fetch candidate email + display name
    // ----------------------------
    const { rows: candRows } = await sbFetch(
      env,
      `${env.SUPABASE_URL}/rest/v1/candidates?id=eq.${encodeURIComponent(candId)}&select=id,email,display_name,first_name,last_name`
    );
    if (!candRows.length) return withCORS(env, req, notFound("Candidate not found"));
    const cand = candRows[0];
    const toEmail = (cand.email || '').trim();
    ...
    const candName = cand.display_name ||
                     [cand.first_name, cand.last_name].filter(Boolean).join(' ') ||
                     'Candidate';

    // ----------------------------
    // Prepare period label & reference key
    // ----------------------------
    const dates = finRows
      .map(r => r?.timesheet?.week_ending_date)
      .filter(Boolean)
      .sort();
    ...
    // ----------------------------
    // Audit trail: candidate + each timesheet
    // ----------------------------
    const mailId = mail?.id || null;
...
async function handleInvoiceEmail(env, req, invoiceId) {
  // Queue into mail_outbox per Brief (Power Automate picks it up)
  const user = await requireUser(env, req, ['admin']);
  if (!user) return unauthorized();
  ...
  try {
    const { rows } = await sbFetch(
      env,
      `${env.SUPABASE_URL}/rest/v1/invoices?id=eq.${encodeURIComponent(invoiceId)}&select=invoice_no,invoice_pdf_r2_key,client:clients(primary_invoice_email,name)`
    );
    if (!rows.length) return withCORS(env, req, notFound("Invoice not found"));
    const inv = rows[0];
    const to = inv.client?.primary_invoice_email || null;
    ...
    // Queue in mail_outbox (return representation so we can audit with the mail id)
    const out = await fetch(`${env.SUPABASE_URL}/rest/v1/mail_outbox`, {
      method: "POST",
      headers: { ...sbHeaders(env), Prefer: "return=representation" },
      body: JSON.stringify({
    ...
    // Audit: record that this invoice email was queued (include the exact PDF key sent)
    await writeAudit(
      env,
      user,
      'EMAIL_QUEUED',
      { to, subject, invoice_pdf_r2_key: pdfKey, mail_id: mailId },
