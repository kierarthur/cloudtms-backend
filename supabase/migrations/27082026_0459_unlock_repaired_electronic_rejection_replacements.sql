-- Complete the one-time electronic rejection replacement repair by restoring
-- the empty replacement's financial lifecycle to UNPROCESSED.  The preceding
-- repair changed the route back to ELECTRONIC but intentionally preserved a
-- concurrently projected PENDING_AUTH value; that stale value still made the
-- otherwise-empty replacement Candidate-immutable.
...
begin
  ...
  loop
    ...
    where id=v_row.financials_id
      ...
      and is_current=true
      and processing_status='PENDING_AUTH'
      and authorised_at_utc is null
      and paid_at_utc is null
      and locked_by_invoice_id is null;
    if not found then
      raise exception 'CANDIDATE_ELECTRONIC_REJECTION_UNLOCK_CONFLICT'
        using errcode='40001';
    end if;
    ...
    insert into public.audit_events(
      ...
      'CANDIDATE_ELECTRONIC_REJECTION_REPLACEMENT_UNLOCKED',
      jsonb_build_object('processing_status',v_row.prior_processing_status),
      jsonb_build_object('processing_status','UNPROCESSED'),
      'Restore empty repaired electronic rejection replacement to Candidate-editable lifecycle',
      'ELECTRONIC-REJECTION-UNLOCK:'||v_row.timesheet_id::text,
      clock_timestamp()
    );
  end loop;
end;
