-- Keep the Loans / Snoozes register aligned with the live Banking Pay resolution rule.
-- A taxable component needs operator resolution only for an actionable pay-channel switch.
         ...
         SELECT pfc.finance_case_id,
            ...
            (count(*) FILTER (
              WHERE pfc.closed_at_utc IS NULL
                ...
                AND upper(COALESCE(pfc.source_pay_method, ''::text)) IS DISTINCT FROM upper(COALESCE(component_candidate.pay_method, ''::text))
                AND (
                  component_case.case_type = 'MANUAL_DEBT_ADJUSTMENT'::pay_finance_case_type_enum
                  OR (
                    CASE
                      WHEN NULLIF(pfc.source_basis_json ->> 'source_units'::text, ''::text) ~ '^-?\d+(\.\d+)?$'::text
                      THEN (NULLIF(pfc.source_basis_json ->> 'source_units'::text, ''::text))::numeric <> (0)::numeric
                      ELSE false
                    END
                    AND NULLIF(pfc.source_basis_json ->> 'source_rate'::text, ''::text) ~ '^-?\d+(\.\d+)?$'::text
                    AND NULLIF(pfc.source_basis_json ->> 'source_charge_rate'::text, ''::text) ~ '^-?\d+(\.\d+)?$'::text
                    AND COALESCE(pfc.component_key_type, ''::text) <> 'ADJUSTMENT_CODE'::text
                  )
                )
                AND (
                  pfc.is_resolution_stale
                  OR pfc.saved_target_pay_method IS NULL
                  OR upper(COALESCE(pfc.saved_target_pay_method, ''::text)) IS DISTINCT FROM upper(COALESCE(component_candidate.pay_method, ''::text))
           ...
           JOIN pay_advances component_case
             ON component_case.id = pfc.finance_case_id
           JOIN candidates component_candidate
             ON component_candidate.id = pfc.candidate_id
