...
create or replace function public._inv_lock_segments_for_invoice(
  p_invoice_id uuid,
...
begin
  ...
  loop
    ...
    select
      bool_or(nullif(btrim(coalesce(x->>'segment_id','')), '') is null) as lock_whole,
    ...
    then
      ...
      loop
        ...
        if v_lock_whole or (v_sid <> '' and v_sid = any(v_seg_ids)) then
          if v_locked_text is null then
            v_locked_text := p_invoice_id::text;
      ...
      -- Non-segments: only lock whole if requested and not already locked
      if v_lock_whole and r_tf.locked_by_invoice_id is null then
        update public.timesheets_financials tfu
        set
          updated_at = v_now,
