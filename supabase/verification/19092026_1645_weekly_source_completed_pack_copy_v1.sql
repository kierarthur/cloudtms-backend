...
-- The managed TEST release owner cannot SET ROLE service_role.  Prove the
-- service grant and direct-table denial here; a separate local PostgreSQL
-- service-role execution and the actual TEST Worker prove the runtime path.
select pg_temp.assert_true(
...
select pg_temp.assert_true(
  ...
  and not has_function_privilege('authenticated',
    'private._weekly_source_completed_pack_copy_claim_event_valid_v1(public.mail_outbox)',
    'EXECUTE'),
  'completed-copy claim predicate has incorrect execution grants'
);
