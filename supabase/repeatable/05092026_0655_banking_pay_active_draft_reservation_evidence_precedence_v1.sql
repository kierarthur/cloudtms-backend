-- Exact replacement authority for cancelled-Draft reservation evidence precedence.
-- Historical owner 01092026_1459 remains byte-identical.
-- This changes no amount, economic key, PAYE/Umbrella, VAT, reservation,
-- selection, provider, payment, settlement, remittance or Policy X owner.
-- An exact allocation-row-backed top-level frozen source basis supersedes only
-- its byte-equivalent ordinary case-component compatibility view. Genuine
-- distinct/malformed evidence retains the established fail-closed guards.
...
AS $function$
  WITH build_authority AS MATERIALIZED (
    ...
    SELECT raw.*,
      ...
      UPPER(NULLIF(BTRIM(COALESCE(
        raw.evidence_json->>'component_key_type',
        raw.evidence_json#>>'{source_basis_json,component_key_type}',
        raw.evidence_json->>'key_type',
        raw.evidence_json#>>'{source_basis_json,key_type}')),''))
        AS nested_economic_key_type,
      NULLIF(BTRIM(COALESCE(
        raw.evidence_json->>'component_key_value',
        raw.evidence_json#>>'{source_basis_json,component_key_value}',
        raw.evidence_json->>'key_value',
        raw.evidence_json#>>'{source_basis_json,key_value}')),'')
        AS nested_economic_key_value,
      CASE WHEN COALESCE(raw.evidence_json->>'timesheet_id',
          raw.evidence_json#>>'{source_basis_json,timesheet_id}','')
          ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        THEN COALESCE(raw.evidence_json->>'timesheet_id',
          raw.evidence_json#>>'{source_basis_json,timesheet_id}')::uuid END
        AS nested_timesheet_id,
      CASE WHEN COALESCE(raw.evidence_json->>'allocation_row_id',
          raw.evidence_json#>>'{source_basis_json,allocation_row_id}','')
          ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
        THEN COALESCE(raw.evidence_json->>'allocation_row_id',
          raw.evidence_json#>>'{source_basis_json,allocation_row_id}')::uuid END
        AS nested_allocation_row_id
    ...
    -- The exact top-level allocation-backed identity is the authoritative
    -- representation of an otherwise byte-equivalent ordinary compatibility
    -- component.  Build that finite identity set once.  The prior correlated
    -- lookup rescanned every evidence row for every ordinary component and
    -- made the unchanged decision path quadratic at large Draft scale.
    SELECT DISTINCT specific.fact_identity,
      jsonb_build_array(
        specific.allocated_amount_ex_vat,
        specific.nested_economic_key_type,
        specific.nested_economic_key_value,
        specific.nested_timesheet_id,
        specific.component_member_identity,
        specific.bucket_code
      ) AS precedence_key_json,
      specific.evidence_source_pay_method
    FROM nested_evidence_digest specific
    WHERE specific.evidence_origin='TOP_LEVEL_SOURCE_BASIS'
      AND specific.allocation_claimed
      AND specific.allocated_amount_ex_vat IS NOT NULL
      AND specific.nested_allocation_row_id IS NOT NULL
      AND specific.source_basis_json->>'source'=
        'banking_pay_workbench_preview_rows'
      AND specific.nested_economic_key_type=specific.economic_key_type
      AND specific.nested_economic_key_value=specific.economic_key_value
      AND specific.nested_timesheet_id=specific.timesheet_id
      AND COALESCE(specific.source_basis_json->>'source_reservation_amount_ex_vat','')
        ~ '^-?\d+(\.\d+)?$'
      AND ROUND((specific.source_basis_json->>'source_reservation_amount_ex_vat')::numeric,2)=
        specific.allocated_amount_ex_vat
      AND ROUND(specific.parent_amount_ex_vat,2)=
        specific.allocated_amount_ex_vat
  ), nested_top_level_allocation_precedence_any_method AS MATERIALIZED (
    SELECT DISTINCT precedence.fact_identity,precedence.precedence_key_json
    FROM nested_top_level_allocation_precedence precedence
    WHERE precedence.evidence_source_pay_method IS NULL
  ), nested_top_level_allocation_precedence_exact_method AS MATERIALIZED (
    SELECT DISTINCT precedence.fact_identity,
      precedence.precedence_key_json||jsonb_build_array(precedence.evidence_source_pay_method)
        AS precedence_key_json
    FROM nested_top_level_allocation_precedence precedence
    WHERE precedence.evidence_source_pay_method IS NOT NULL
  ), nested_evidence_preferred AS MATERIALIZED (
    SELECT normalized.*
    FROM nested_evidence_digest normalized
    LEFT JOIN nested_top_level_allocation_precedence_any_method precedence_any
      ON precedence_any.fact_identity=normalized.fact_identity
     AND precedence_any.precedence_key_json=jsonb_build_array(
       normalized.allocated_amount_ex_vat,
       normalized.nested_economic_key_type,
       normalized.nested_economic_key_value,
       normalized.timesheet_id,
       normalized.component_member_identity,
       normalized.bucket_code)
    LEFT JOIN nested_top_level_allocation_precedence_exact_method precedence_exact
      ON precedence_exact.fact_identity=normalized.fact_identity
     AND precedence_exact.precedence_key_json=jsonb_build_array(
       normalized.allocated_amount_ex_vat,
       normalized.nested_economic_key_type,
       normalized.nested_economic_key_value,
       normalized.timesheet_id,
      ...
      (normalized.evidence_origin='CASE_COMPONENT'
        AND EXISTS(
          ...
          WHERE specific.fact_identity=normalized.fact_identity
            ...
            AND specific.allocation_claimed
            AND specific.allocated_amount_ex_vat IS NOT NULL))
      OR (normalized.evidence_origin='CASE_COMPONENT'
        AND normalized.allocation_claimed
        AND normalized.allocated_amount_ex_vat IS NOT NULL
        AND UPPER(NULLIF(BTRIM(normalized.evidence_json->>'classification'),''))
          ='ORDINARY'
        AND normalized.component_fallback IS NULL
        AND normalized.evidence_json->'saved_resolution_payload_json' IS NULL
        AND normalized.nested_economic_key_type=normalized.economic_key_type
        AND normalized.nested_economic_key_value=normalized.economic_key_value
        AND normalized.evidence_source_pay_method=normalized.sealed_source_pay_method
        AND (precedence_any.fact_identity IS NOT NULL
          OR precedence_exact.fact_identity IS NOT NULL))
