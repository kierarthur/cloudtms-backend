...
-- -----------------------------------------------------------------------------
-- public.pay_workbench_worker_drain_chunk(p_limit integer, p_now_utc timestamp with time zone, p_session_id uuid, p_candidate_id uuid, p_allowed_job_types text[], p_worker_id text, p_lease_seconds integer)
-- Installed pg_get_functiondef MD5: 2cd2971ca90b18e2208a772fb3503b0c
-- -----------------------------------------------------------------------------
    ...
    BEGIN
      ...
      ELSE
        ...
        ELSIF v_canonical_job_type = 'WORKBENCH_CANDIDATE_SOURCE_BUILD' THEN
          RAISE EXCEPTION 'PAY_WORKBENCH_MATERIAL_SOURCE_REQUIRES_TWO_CALL_PROTOCOL'
            USING ERRCODE='55000';
...
ALTER FUNCTION public.pay_workbench_worker_drain_chunk(p_limit integer, p_now_utc timestamp with time zone, p_session_id uuid, p_candidate_id uuid, p_allowed_job_types text[], p_worker_id text, p_lease_seconds integer) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.pay_workbench_worker_drain_chunk(p_limit integer, p_now_utc timestamp with time zone, p_session_id uuid, p_candidate_id uuid, p_allowed_job_types text[], p_worker_id text, p_lease_seconds integer) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.pay_workbench_worker_drain_chunk(p_limit integer, p_now_utc timestamp with time zone, p_session_id uuid, p_candidate_id uuid, p_allowed_job_types text[], p_worker_id text, p_lease_seconds integer) TO postgres;
GRANT EXECUTE ON FUNCTION public.pay_workbench_worker_drain_chunk(p_limit integer, p_now_utc timestamp with time zone, p_session_id uuid, p_candidate_id uuid, p_allowed_job_types text[], p_worker_id text, p_lease_seconds integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.pay_workbench_worker_drain_chunk(p_limit integer, p_now_utc timestamp with time zone, p_session_id uuid, p_candidate_id uuid, p_allowed_job_types text[], p_worker_id text, p_lease_seconds integer) TO service_role;
