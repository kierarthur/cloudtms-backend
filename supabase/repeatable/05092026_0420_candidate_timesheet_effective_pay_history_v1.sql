-- Repeatable CloudTMS function authority: candidate_timesheet_effective_pay_history
-- Candidate Current/History and detail status use the same settled-payment
-- precedence as the established Office Timesheet summary. A partial,
-- processing or outstanding adjustment stays Current.
...
alter function public.candidate_app_timesheet_detail_v1(
  uuid,text,uuid,uuid,uuid,timestamptz
) owner to postgres;
revoke all on function public.candidate_app_timesheet_detail_v1(
  uuid,text,uuid,uuid,uuid,timestamptz
) from public,anon,authenticated;
grant execute on function public.candidate_app_timesheet_detail_v1(
  uuid,text,uuid,uuid,uuid,timestamptz
) to service_role;
