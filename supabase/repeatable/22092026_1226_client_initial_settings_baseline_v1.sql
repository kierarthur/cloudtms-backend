...
CREATE OR REPLACE FUNCTION public.client_create_with_settings_v1(p_client_id uuid, p_client_json jsonb, p_actor_user_id uuid, p_settings_json jsonb DEFAULT '{}'::jsonb, p_now_utc timestamp with time zone DEFAULT now())
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF FOUND THEN
    ...
    -- The initial effective date is server-owned, including on exact retries.
    -- Preserve legacy replay comparison for clients created before this rule.
    WHERE NOT (requested_key.key_name = 'effective_from'
               AND v_existing_settings.effective_from = DATE '1900-01-01')
      AND (to_jsonb(v_existing_settings) -> requested_key.key_name)
          IS DISTINCT FROM
  ...
  ELSE
    ...
    VALUES (
      ...
      -- Initial settings cover backdated contracts; later revisions keep their dates.
      DATE '1900-01-01',
      v_now,
      v_now,
...
alter function public.client_create_with_settings_v1(uuid,jsonb,uuid,jsonb,timestamptz) owner to postgres;
...
comment on function public.client_create_with_settings_v1(uuid,jsonb,uuid,jsonb,timestamptz) is
  'Atomically creates a client and its initial settings. Initial settings are effective from 1900-01-01; later revisions retain their own effective dates.';
