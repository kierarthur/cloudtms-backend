...
-- WP-20 (WP-11a finding F9 / handoff N3, endorsed by WP-11a_REVIEW.md F9).
-- The duplicate `public.candidate_app_timesheet_detail_v2` that stood here was
-- byte-identical to the copy this file is not the owner of. Its owner is
-- `02092026_0325_candidate_paper_break_entry_v1.sql`, which applies later by the
-- runner's re-keyed DDMMYYYY order and is the only place the reader is edited.
--
-- Keeping the duplicate here was a live release hazard rather than a latent one:
-- `09092026_1500_candidate_duplicate_expense_anchor_final_authority_v1.sql`
-- (2026-09-09) re-includes this file through
-- `08092026_0631_candidate_duplicate_expense_anchor_inclusion_v1.sql` ->
-- `27082026_1436_candidate_withdrawal_read_authority_v1.sql`, so on a build from
-- empty this stale copy was reinstalled AFTER the real owner and silently
-- reverted it. Proved by dumping the installed definition after a full NEW build
-- from empty: the additive Weekly Source call was absent (0 occurrences).
-- Removing it leaves exactly one owner, which is what the release order already
-- assumes. The Daily projection subject of this file is unaffected: nothing here
-- is the detail reader's owner.
