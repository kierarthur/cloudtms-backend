-- Repeatable CloudTMS function authority: candidate_processed_action_projection_v1
-- Keeps submitted/processed hours immutable while permitting only a separately allocated Candidate expense carrier.
...
comment on function private._candidate_record_capabilities_v1(uuid,uuid,jsonb) is
  'Single server capability authority for role, editors, ATTACH choices, preview border, processing, rejection and expense placement; submitted, processed or authorised hours are immutable and may anchor only a separately allocated expense carrier.';
