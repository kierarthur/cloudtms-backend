-- Project a verified control-plane Candidate session without allowing one
-- device/login family to invalidate an independent valid family for the same
...
create or replace function public.candidate_app_federated_session_project_v2(
  p_environment text,
  ...
  p_global_session_family_identity_hmac bytea
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_latest public.candidate_app_sessions%rowtype;
...
begin
  if v_environment not in ('TEST','LIVE')
     ...
     or coalesce(pg_catalog.octet_length(p_global_session_family_identity_hmac),0)<>32
  ...
  -- Epochs are comparable only inside one stable control-plane login family.
  -- Durable history from that family still blocks delayed older requests.
  select * into v_latest
  from public.candidate_app_sessions
  where auth_source='CONTROL_PLANE'
    and membership_id=p_membership_id
    and global_session_family_identity_hmac=p_global_session_family_identity_hmac
  order by membership_generation desc,route_version desc,session_epoch desc,
           created_at_utc desc,id desc
  limit 1
  for update;
  if found and (
       v_latest.membership_generation>p_membership_generation
       or (v_latest.membership_generation=p_membership_generation
         and v_latest.route_version>p_route_version)
       or (v_latest.membership_generation=p_membership_generation
         and v_latest.route_version=p_route_version
         and v_latest.session_epoch>p_session_epoch)
       or (v_latest.membership_generation=p_membership_generation
         and v_latest.route_version=p_route_version
         and v_latest.session_epoch=p_session_epoch
         and v_latest.global_session_identity_hmac<>p_global_session_identity_hmac)
     ) then
    raise exception 'CANDIDATE_FEDERATED_SESSION_STALE' using errcode='28000';
  end if;

  update public.candidate_app_sessions
  set status='REVOKED',revoked_at_utc=p_now_utc,
      revoke_reason='FEDERATED_CONTEXT_STALE',updated_at_utc=p_now_utc
  where auth_source='CONTROL_PLANE' and membership_id=p_membership_id and status='ACTIVE'
    and (
      membership_generation<>p_membership_generation
      ...
      or global_session_family_identity_hmac is null
      or (
        global_session_family_identity_hmac=p_global_session_family_identity_hmac
        and (
          session_epoch<>p_session_epoch
  ...
  if found then
    update public.candidate_app_sessions
    set expires_at_utc=v_expiry,
        absolute_expires_at_utc=v_expiry,
        last_used_at_utc=p_now_utc,
  ...
  insert into public.candidate_app_sessions(
    ...
    global_session_family_identity_hmac,membership_id,membership_generation,route_version,
    session_epoch,created_at_utc,updated_at_utc
  ) values (
    ...
    p_global_account_identity_hmac,p_global_session_identity_hmac,
    p_global_session_family_identity_hmac,p_membership_id,p_membership_generation,
    p_route_version,p_session_epoch,p_now_utc,p_now_utc
  ) returning * into v_existing;
...
revoke all on function public.candidate_app_federated_session_project_v2(
  text,bytea,bytea,uuid,integer,uuid,integer,bigint,timestamptz,timestamptz,bytea
) from public,anon,authenticated;
grant execute on function public.candidate_app_federated_session_project_v2(
  text,bytea,bytea,uuid,integer,uuid,integer,bigint,timestamptz,timestamptz,bytea
) to service_role;

comment on function public.candidate_app_federated_session_project_v2(
  text,bytea,bytea,uuid,integer,uuid,integer,bigint,timestamptz,timestamptz,bytea
) is 'Projects a verified control-plane Candidate session with monotonicity scoped to its stable login family, allowing independent valid device families without weakening stale-request rejection.';
