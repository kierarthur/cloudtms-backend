-- The Candidate expense route is part of the generator's source identity.
-- Reproduce that exact identity during reference validation so unchanged sources
-- do not fail as SOURCE_REVISION_CHANGED.

create or replace function private._invoice_source_reference_validate_batch(
...
authoritative as materialized (
  select s.*,t.sheet_scope::text sheet_scope,
    ...
    encode(digest(concat_ws('|',
      ...
      s.invoice_stream,s.consolidation_mode,
      case when private._candidate_feature_enabled_current_v1('candidate_expense_invoice_routing_v1')
        then coalesce(case when s.invoice_stream='EXPENSE' then lower(nullif(btrim(
          coalesce(a.authority_json#>>'{values,candidate_expense_invoice_email}','')
        ),'')) end,'NO_EXPENSE_RECIPIENT') end),'sha256'),'hex')
      current_revision
  from supplied s
