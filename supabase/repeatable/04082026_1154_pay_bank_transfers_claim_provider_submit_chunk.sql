...
BEGIN
  ...
  IF coalesce(v_same_operation_authorised_auth_count, 0) <= 0 THEN
    RETURN jsonb_build_object(
      ...
      'processed_count', 0,
      'status', 'BLOCKED',
      'blocker_code', 'AUTH_REQUEST_NOT_AUTHORISED_FOR_OPERATION',
      'display_message', 'Payment authorisation is not valid for this operation.',
      'chunk_id', NULL,
      'transfer_scope_ids', '[]'::jsonb,
      'auth_request_ids', '[]'::jsonb,
      'transfers', '[]'::jsonb,
  ...
  IF coalesce(v_claimed_count, 0) <= 0 THEN
    ...
    RETURN jsonb_build_object(
      ...
      'processed_count', 0,
      'status', 'NO_SCOPE_DUE',
      'blocker_code', 'PROVIDER_SUBMIT_NO_ELIGIBLE_TRANSFERS',
      'display_message', 'No payment instructions are ready to submit.',
      'chunk_id', NULL,
      'transfer_scope_ids', '[]'::jsonb,
  ...
  RETURN jsonb_build_object(
    ...
    'operation_id', p_operation_id::text,
    'pay_batch_id', p_pay_batch_id::text,
    'processed_count', coalesce(v_claimed_count, 0),
    'status', 'CLAIMED',
    'blocker_code', NULL,
    'display_message', 'Payment instructions claimed for bounded submission.',
...
END;
$function$;
ALTER FUNCTION public.pay_bank_transfers_claim_provider_submit_chunk(uuid,uuid,integer,text,integer) OWNER TO postgres;
REVOKE ALL ON FUNCTION public.pay_bank_transfers_claim_provider_submit_chunk(uuid,uuid,integer,text,integer) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.pay_bank_transfers_claim_provider_submit_chunk(uuid,uuid,integer,text,integer) FROM anon;
REVOKE ALL ON FUNCTION public.pay_bank_transfers_claim_provider_submit_chunk(uuid,uuid,integer,text,integer) FROM authenticated;
REVOKE ALL ON FUNCTION public.pay_bank_transfers_claim_provider_submit_chunk(uuid,uuid,integer,text,integer) FROM service_role;
GRANT EXECUTE ON FUNCTION public.pay_bank_transfers_claim_provider_submit_chunk(uuid,uuid,integer,text,integer) TO service_role;
