-- Candidate returned-paper QR proof transaction boundary.
-- The proof deliberately locks the Candidate session while it verifies the current
-- workflow, manifest and Timesheet QR token. PostgreSQL therefore requires the
-- public function to be VOLATILE rather than STABLE. No business rule changes.
