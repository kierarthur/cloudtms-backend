...
create or replace function private._candidate_daily_source_candidate_bind_on_generation_v1(
  p_environment text,
  p_candidate_global_key text,
...
revoke all on function private._candidate_daily_source_candidate_bind_on_generation_v1(text,text,text,integer)
  from public;
...
revoke all on function private._candidate_daily_source_candidate_bind_on_generation_v1(text,text,text,integer)
  from public;
revoke all on function public.candidate_daily_rota_generation_publish_atomic_v1(jsonb,uuid,text,jsonb,text)
  from public;
...
begin
  ...
  if exists(select 1 from pg_roles where rolname='service_role') then
    revoke all on function private._candidate_daily_source_candidate_bind_on_generation_v1(text,text,text,integer)
      from service_role;
    grant execute on function public.candidate_daily_rota_generation_publish_atomic_v1(jsonb,uuid,text,jsonb,text)
      to service_role;
  end if;
end;
$acl$;
