...
declare
  v_text text:=pg_catalog.btrim(
    pg_catalog.replace(pg_catalog.replace(coalesce(p_text,''),E'\r\n',E'\n'),E'\r',E'\n')
  );
begin
...
begin
  if p_expected_version is null or p_expected_version<1
     ...
     or pg_catalog.char_length(pg_catalog.btrim(coalesce(p_idempotency_key,''))) not between 16 and 200
  then
...
begin
  ...
  return pg_catalog.jsonb_build_object(
    ...
    'entitlements',pg_catalog.jsonb_build_object(
      ...
      'daily',nullif(pg_catalog.btrim(coalesce(v_candidate.key_norm,'')),'') is not null,
      'gck_present',nullif(pg_catalog.btrim(coalesce(v_candidate.key_norm,'')),'') is not null
    ),
