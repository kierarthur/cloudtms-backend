-- Final Candidate expense-carrier creation authority. A reused carrier can
-- retain immutable workflow history that already identifies its one worked
-- Timesheet. Recover that link before same-week cardinality and never guess.
--
-- A separate expense carrier is storage only.  Workflow creation must inherit
-- route authority from the immutable worked Timesheet that the claim belongs
-- to, just as finalisation already does.  This prevents a newly created MANUAL
-- carrier from falsely rejecting an ELECTRONIC expense workflow when printed
-- fallback is available.  Rejected-claim anchor preservation, Daily PHONE
-- recovery and every other transition branch remain aligned with the current
