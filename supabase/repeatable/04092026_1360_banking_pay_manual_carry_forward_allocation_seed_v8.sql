-- CloudTMS Banking Pay manual carry-forward allocation handoff.
  ...
  WITH scope_selected_lines AS (
    ...
    SELECT
      ...
      (
        ...
        AND NULLIF(BTRIM(COALESCE(scope_selected_lines.line_json->>'manual_adjustment_carry_forward_id', '')), '') ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
