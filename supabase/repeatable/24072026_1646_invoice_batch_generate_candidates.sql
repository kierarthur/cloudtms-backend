...
BEGIN
  IF to_regprocedure('private._invoice_batch_generate_candidates_legacy_20260726(boolean, integer, text[])') IS NULL THEN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'invoice_batch_generate_candidates'
      AND pg_get_function_identity_arguments(p.oid) = 'p_allow_early boolean, p_limit integer, p_scope_keys text[]';

    IF v_def IS NULL THEN
      RAISE EXCEPTION 'Cannot preserve legacy public.invoice_batch_generate_candidates(boolean, integer, text[]): source function not found and private legacy copy missing';
    END IF;

    IF position('_invoice_batch_generate_candidates_legacy_20260726' IN v_def) > 0 THEN
      RAISE EXCEPTION 'Refusing to clone an already-wrapped public.invoice_batch_generate_candidates; private legacy copy is missing';
    END IF;

    v_def := regexp_replace(
      v_def,
      '^CREATE OR REPLACE FUNCTION public\.invoice_batch_generate_candidates',
      'CREATE OR REPLACE FUNCTION private._invoice_batch_generate_candidates_legacy_20260726'
    );

    EXECUTE v_def;
    EXECUTE 'REVOKE ALL ON FUNCTION private._invoice_batch_generate_candidates_legacy_20260726(boolean, integer, text[]) FROM PUBLIC, anon, authenticated';
    EXECUTE 'GRANT EXECUTE ON FUNCTION private._invoice_batch_generate_candidates_legacy_20260726(boolean, integer, text[]) TO service_role';
  END IF;
END
$preserve_legacy$;

DROP FUNCTION IF EXISTS public.invoice_batch_generate_candidates(boolean, integer, text[]);

CREATE OR REPLACE FUNCTION public.invoice_batch_generate_candidates(
  p_allow_early boolean DEFAULT false,
  p_limit integer DEFAULT 5000,
  p_scope_keys text[] DEFAULT NULL::text[],
...
REVOKE ALL ON FUNCTION public.invoice_batch_generate_candidates(boolean, integer, text[], jsonb) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.invoice_batch_generate_candidates(boolean, integer, text[], jsonb) TO authenticated, service_role;
