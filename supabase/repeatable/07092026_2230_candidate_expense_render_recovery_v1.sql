...
drop function if exists public.candidate_expense_update_render_recovery_list_v1(
...
begin
  ...
  from (
    ...
    join public.candidate_expense_operations operation
...
alter function public.candidate_expense_update_render_recovery_list_v1(text,integer,uuid,uuid,timestamptz)
  owner to postgres;
revoke all on function public.candidate_expense_update_render_recovery_list_v1(text,integer,uuid,uuid,timestamptz)
  from public,anon,authenticated,service_role;
grant execute on function public.candidate_expense_update_render_recovery_list_v1(text,integer,uuid,uuid,timestamptz)
  to service_role;

comment on function public.candidate_expense_update_render_recovery_list_v1(text,integer,uuid,uuid,timestamptz) is
  'Returns a bounded service-only list of exact saved review renders that outlived the HTTP waitUntil window.';
