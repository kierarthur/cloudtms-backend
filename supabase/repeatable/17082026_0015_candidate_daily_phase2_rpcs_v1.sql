...
declare v_context jsonb; v_environment text; v_request_hash text;
  ...
  v_actor uuid; v_error text; v_link jsonb; v_link_group uuid;
begin
  v_context:=private._candidate_daily_context_v1(p_internal_context,'SIGNED_SYSTEM_SYNC',false);
  v_environment:=v_context->>'environment';
  begin v_actor:=nullif(p_internal_context->>'actor_user_id','')::uuid; exception when others then v_actor:=null; end;
  if p_batch_request_id is null or p_independent_approver is null or p_independent_approver=v_actor
  ...
  for v_item in select value from jsonb_array_elements(p_transition_items) loop
    ...
    begin
      ...
      if v_new_mode not in ('GOOGLE_PRIMARY','ROLLBACK_PENDING','SUPABASE_PRIMARY')
         or coalesce(v_item->>'in_flight_disposition','NONE') not in ('DRAINED','CANCELLED','RECONCILED','NONE') then
        raise exception using errcode='22023',message='VALIDATION_FAILED';
      end if;
      insert into private.candidate_daily_authority_scopes(environment,candidate_id,authority_mode,canonical_version)
      values(v_environment,v_candidate_id,'GOOGLE_PRIMARY',0) on conflict do nothing;
      select * into v_scope from private.candidate_daily_authority_scopes
        where environment=v_environment and candidate_id=v_candidate_id for update;
      v_prior_mode:=v_scope.authority_mode;
      if nullif(v_item->>'expected_authority_mode','') is not null
         and upper(v_item->>'expected_authority_mode')<>v_prior_mode then
        raise exception using errcode='40001',message='SEMANTIC_REJECTION';
      end if;
      if nullif(v_item->>'expected_canonical_version','') is not null
         and (v_item->>'expected_canonical_version')::bigint<>v_scope.canonical_version then
        raise exception using errcode='40001',message='SEMANTIC_REJECTION';
      end if;
      ...
      select coalesce(e.enabled,false) into v_entitlement_before from private.candidate_daily_entitlements e
        where e.environment=v_environment and e.candidate_id=v_candidate_id;
      v_entitlement_after:=coalesce((v_item->>'entitlement_enabled')::boolean,v_entitlement_before);
      v_link:=v_item->'source_link';
      if v_link is not null then
        if jsonb_typeof(v_link)<>'object' or v_link->>'identifier_hmac' !~ '^[a-f0-9]{64}$'
           or coalesce((v_link->>'hmac_key_version')::integer,0)<=0 then
          raise exception using errcode='22023',message='VALIDATION_FAILED';
        end if;
        ...
        values(v_environment,v_candidate_id,'GOOGLE_CREDENTIALLY_PUBLIC_ID','SOURCE_IDENTITY_V1',v_link_group,
          ...
          coalesce(nullif(v_link->>'state',''),'PRIMARY'),v_batch.batch_receipt_id,v_actor,
      ...
      select v_transition_id,v_batch.batch_receipt_id,v_environment,v_candidate_id,v_prior_mode,v_new_mode,now(),
        v_scope.canonical_version,v_scope.active_generation_id,g.generation_version,
        coalesce((select to_jsonb(ss) from private.candidate_daily_sync_state ss where ss.environment=v_environment
          and ss.candidate_id=v_candidate_id and ss.target='MASTER_AVAILABILITY_SHEET'),'{}'::jsonb),
        coalesce(v_item->>'in_flight_disposition','NONE'),v_entitlement_before,v_entitlement_after,v_actor,
        p_independent_approver,p_reason,p_evidence_sha256,'COMMITTED',p_correlation_id
      from (values(1))x(n) left join public.candidate_daily_rota_generations g on g.generation_id=v_scope.active_generation_id;
    ...
    exception when others then
      v_error:=sqlerrm;
      if v_error in ('VALIDATION_FAILED','SEMANTIC_REJECTION','SOURCE_EVENT_CONFLICT') then
        v_outcomes:=v_outcomes||jsonb_build_array(jsonb_build_object('index',v_index,'status','REJECTED','error_code',v_error));
      else raise; end if;
    end;
    v_index:=v_index+1;
