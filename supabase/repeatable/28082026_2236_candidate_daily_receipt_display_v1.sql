-- Daily receipt display: read-only factual hours and existing manager decision.
...
begin
  ...
  -- An unresolved Candidate receipt can already have a genuine zero-valued
  -- financial placeholder. Display its submitted factual hours, not that zero.
  -- Resolved, protected and import-owned records retain financial precedence.
  v_hours:=case when coalesce((v_context->>'candidate_first_receipt')::boolean,false)
      and coalesce((v_context->>'office_resolution_pending')::boolean,false)
      and not v_import and not v_protected
    then coalesce(v_ts.worked_minutes::numeric/60,v_fin.total_hours,0)
    else coalesce(v_fin.total_hours,v_ts.worked_minutes::numeric/60,0) end;
...
revoke all on function private._candidate_daily_read_projection_v1(text,uuid,uuid,timestamptz) from public,anon,authenticated,service_role;
