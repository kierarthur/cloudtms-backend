-- A rejected expenses-only claim must reopen against the immutable worked
-- Timesheet that originally authorised the separate expense route.  The
-- freshly rotated expense carrier is deliberately empty and must never be
-- mistaken for the worked anchor.  Daily interrupted PHONE recovery and every
-- other transition branch remain byte-for-byte aligned with the current
