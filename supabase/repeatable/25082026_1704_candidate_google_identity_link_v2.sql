...
begin
  ...
  select pg_catalog.count(*)::integer,
         ...
         pg_catalog.min(pg_catalog.upper(pg_catalog.btrim(pg_catalog.coalesce(c.key_norm,''))))
    into v_count,v_candidate_id,v_existing_code
  from public.candidates c
  where c.active is true
    and pg_catalog.lower(pg_catalog.btrim(pg_catalog.coalesce(c.last_name,'')))=v_surname
  ...
  v_code_state:=case
    when pg_catalog.nullif(v_existing_code,'') is null then 'UNASSIGNED'
...
begin
  ...
  if not v_candidate.active
     or pg_catalog.lower(pg_catalog.btrim(pg_catalog.coalesce(v_candidate.last_name,'')))<>v_surname
  ...
  v_existing:=pg_catalog.upper(pg_catalog.btrim(pg_catalog.coalesce(v_candidate.key_norm,'')));
  if pg_catalog.nullif(v_existing,'') is null then
    update public.candidates c
       set key_norm=v_code,updated_at=p_now_utc
     where c.id=p_local_candidate_id
       and pg_catalog.nullif(pg_catalog.btrim(pg_catalog.coalesce(c.key_norm,'')),'') is null;
    if not found then
