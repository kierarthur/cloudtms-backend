-- H2 F-013 provisional isolated replacement; never publish or install alone.
-- Derived byte-for-byte from the cfa4de12 current owner, source file SHA-256
-- 78191be37ac01ec5f2c6c43d56edc4da3b0ec7a935b73c7d84f894e221c19763.
-- It admits only producer-certified visible finance constituents at the existing
-- scope handoff. It changes no finance equation, channel, tax, VAT or payee rule.
-- F-010a shares this routine and must be combined in one later reviewed owner.
...
BEGIN
  ...
  SELECT
    ...
    (
      selected_base.finance_case_id IS NOT NULL
      AND selected_base.line_type IN (
      ...
      AND (
        (
          selected_base.line_type = 'OVERPAYMENT_RECOVERY'
          AND selected_base.case_type = 'OVERPAYMENT'
          AND selected_base.item_direction = 'DEDUCTION'
          AND ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) < 0
        )
        OR (
          selected_base.line_type = 'MANUAL_DEBT_RECOVERY'
          AND selected_base.case_type = 'MANUAL_DEBT_ADJUSTMENT'
          AND selected_base.item_direction = 'DEDUCTION'
          AND ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) < 0
        )
        OR (
          selected_base.line_type = 'PAYMENT_ADVANCE_REPAYMENT'
          AND selected_base.case_type = 'PAYMENT_ADVANCE'
          AND selected_base.item_direction = 'DEDUCTION'
          AND ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) < 0
        )
        OR (
          selected_base.line_type = 'LOAN_PAYOUT'
          AND selected_base.case_type = 'PAYMENT_ADVANCE'
          AND selected_base.item_direction = 'PAYMENT'
          AND ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) > 0
        )
        OR (
          selected_base.line_type = 'UNDERPAYMENT_PAYMENT'
          AND selected_base.case_type = 'UNDERPAYMENT'
          AND selected_base.item_direction = 'PAYMENT'
          AND ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) > 0
        )
        OR (
          selected_base.line_type = 'MANUAL_CREDIT_ADJUSTMENT_PAYMENT'
          AND selected_base.case_type = 'MANUAL_CREDIT_ADJUSTMENT'
          AND selected_base.item_direction = 'PAYMENT'
          AND ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) > 0
        )
      )
      AND selected_base.key_type IN (
        'TS_DAY', 'TS_TOTAL', 'ADDITIONAL_CODE', 'ADJUSTMENT_CODE',
        'EXPENSE_CODE', 'MANUAL_CARRY_FORWARD', 'CASE_TOTAL', 'FINANCE_COMPONENT'
      )
      AND selected_base.key_value IS NOT NULL
      AND NOT (
        selected_base.key_type = 'TS_DAY'
        AND selected_base.key_value !~ '^\d{4}-\d{2}-\d{2}$'
      )
      AND (
        (
          selected_base.timesheet_id IS NULL
          AND NULLIF(BTRIM(COALESCE(selected_base.row_json#>>'{economic_key,timesheet_id}', '')), '') IS NULL
        )
        OR (
          selected_base.timesheet_id IS NOT NULL
          AND NULLIF(BTRIM(COALESCE(selected_base.row_json#>>'{economic_key,timesheet_id}', '')), '')
                = selected_base.timesheet_id::text
        )
      )
      AND NULLIF(BTRIM(COALESCE(selected_base.row_key, '')), '')
            = 'finance:' || selected_base.finance_case_id::text || ':' || LOWER(selected_base.line_type)
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'preview_row_id', '')), '')
            = 'finance:' || selected_base.finance_case_id::text || ':' || LOWER(selected_base.line_type)
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'line_id', '')), '')
            = 'finance:' || selected_base.finance_case_id::text || ':' || LOWER(selected_base.line_type)
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'case_key', '')), '')
            = 'finance:' || selected_base.finance_case_id::text
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'source_ref', '')), '')
            = 'advance:' || selected_base.finance_case_id::text
      AND jsonb_typeof(selected_base.row_json->'case_components') = 'array'
      AND (
        (
          jsonb_array_length(selected_base.row_json->'case_components') = 1
          AND selected_base.finance_component_id IS NOT NULL
          AND NULLIF(BTRIM(COALESCE(
                selected_base.row_json#>>'{case_components,0,finance_component_id}', ''
              )), '') = selected_base.finance_component_id::text
          AND UPPER(NULLIF(BTRIM(COALESCE(
                selected_base.row_json#>>'{case_components,0,component_key_type}', ''
              )), '')) = selected_base.key_type
          AND NULLIF(BTRIM(COALESCE(
                selected_base.row_json#>>'{case_components,0,component_key_value}', ''
              )), '') = selected_base.key_value
        )
        OR (
          jsonb_array_length(selected_base.row_json->'case_components') <> 1
          AND selected_base.finance_component_id IS NULL
        )
      )
    ) AS is_certified_finance_constituent,
    (
      selected_base.line_type = 'MANUAL_ADJUSTMENT_CARRY_FORWARD'
      AND selected_base.case_type = 'MANUAL_ADJUSTMENT_CARRY_FORWARD'
      AND selected_base.item_direction IN ('CREDIT', 'DEBIT')
      AND selected_base.finance_case_id IS NULL
      AND selected_base.finance_component_id IS NULL
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'manual_adjustment_carry_forward_id', '')), '')
            ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
      AND NULLIF(BTRIM(COALESCE(selected_base.row_key, '')), '')
            = 'carry_forward:' || LOWER(selected_base.row_json->>'manual_adjustment_carry_forward_id')
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'preview_row_id', '')), '')
            = 'carry_forward:' || LOWER(selected_base.row_json->>'manual_adjustment_carry_forward_id')
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'line_id', '')), '')
            = 'carry_forward:' || LOWER(selected_base.row_json->>'manual_adjustment_carry_forward_id')
      ...
      AND NULLIF(BTRIM(COALESCE(selected_base.row_json->>'source_ref', '')), '')
            = 'carry_forward:' || LOWER(selected_base.row_json->>'manual_adjustment_carry_forward_id')
      AND selected_base.key_type = 'MANUAL_CARRY_FORWARD'
      AND selected_base.key_value = LOWER(selected_base.row_json->>'manual_adjustment_carry_forward_id')
      AND (
        (ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) > 0 AND selected_base.item_direction = 'CREDIT')
        OR (ROUND(COALESCE(selected_base.amount_ex_vat, 0), 2) < 0 AND selected_base.item_direction = 'DEBIT')
      )
           ...
           'reasons', COALESCE(invalid_rows.preview_contract_json->'reasons', '[]'::jsonb)
             ...
             || CASE
              WHEN ROUND(COALESCE(invalid_rows.amount_ex_vat, 0), 2) < 0
                ...
                AND COALESCE(invalid_rows.is_certified_manual_carry_forward, false) IS NOT TRUE
                THEN jsonb_build_array('NEGATIVE_ENTITLEMENT_MUST_ROUTE_TO_FINANCE_CASE')
               ELSE '[]'::jsonb
              END
             || CASE
               WHEN invalid_rows.line_type IN (
                 'OVERPAYMENT_RECOVERY',
                 'MANUAL_DEBT_RECOVERY',
                 'PAYMENT_ADVANCE_REPAYMENT',
                 'LOAN_PAYOUT',
                ...
                AND COALESCE(invalid_rows.is_certified_finance_constituent, false) IS NOT TRUE
                 THEN jsonb_build_array('CERTIFIED_FINANCE_CONSTITUENT_IDENTITY_INVALID')
               ELSE '[]'::jsonb
             END
  ...
  FROM (
    ...
    WHERE selected_candidate.rotation_validation_failed IS TRUE
       ...
       OR (
         selected_candidate.line_type IN (
           'OVERPAYMENT_RECOVERY',
           'MANUAL_DEBT_RECOVERY',
           'PAYMENT_ADVANCE_REPAYMENT',
           'LOAN_PAYOUT',
         ...
         AND COALESCE(selected_candidate.is_certified_finance_constituent, false) IS NOT TRUE
       )
       OR (
          COALESCE(selected_candidate.is_certified_finance_constituent, false) IS NOT TRUE
          AND COALESCE(selected_candidate.is_certified_manual_carry_forward, false) IS NOT TRUE
          AND (
           selected_candidate.timesheet_id IS NULL
  ...
  WHERE selected_candidate.rotation_validation_failed IS NOT TRUE
    ...
    AND (
      COALESCE(selected_candidate.is_certified_finance_constituent, false) IS TRUE
      OR COALESCE(selected_candidate.is_certified_manual_carry_forward, false) IS TRUE
      OR (
        COALESCE(selected_candidate.line_type, '') NOT IN (
          'OVERPAYMENT_RECOVERY',
          'MANUAL_DEBT_RECOVERY',
          'PAYMENT_ADVANCE_REPAYMENT',
          'LOAN_PAYOUT',
