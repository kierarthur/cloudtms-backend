-- TEST Banking Loans / Snoozes paged read contract.
--
...
create or replace function public.pay_loans_snoozes_page(
  ...
  p_finance_page_size integer default 20,
  ...
  p_timesheet_page_size integer default 20
)
returns jsonb
language plpgsql
security definer
...
declare
  ...
  v_finance_page_size integer := least(greatest(coalesce(p_finance_page_size, 20), 1), 20);
  v_timesheet_page integer := greatest(coalesce(p_timesheet_page, 1), 1);
  v_timesheet_page_size integer := least(greatest(coalesce(p_timesheet_page_size, 20), 1), 20);
...
begin
  ...
  with ordered_rows as (
    select
      ...
      row_number() over (
        order by
          nullif(src.value->>'created_at_utc', '')::timestamptz desc nulls last,
          lower(coalesce(src.value->>'candidate_display_name', src.value->>'candidate_tms_ref', '')) asc,
...
revoke execute on function public.pay_loans_snoozes_page(
  uuid, uuid, boolean, text, text, text, integer, integer, text, text, integer, integer
) from public, anon, authenticated;

grant execute on function public.pay_loans_snoozes_page(
  uuid, uuid, boolean, text, text, text, integer, integer, text, text, integer, integer
) to service_role;
