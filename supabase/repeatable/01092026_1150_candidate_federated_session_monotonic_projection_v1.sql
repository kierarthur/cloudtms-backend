-- Keep agency-local Candidate session projection monotonic. A delayed request
-- carrying an older, but still cryptographically valid, control-plane context
-- must never revoke the newer session that has already replaced it.
...
begin
  ...
  -- Session epochs are increased by the control plane on every refresh. Keep
  -- the highest observed authority as durable history, including after it has
  -- expired or been revoked. This blocks a slower request from an earlier
  -- epoch from rolling the agency back after a newer request has projected.
...
comment on function public.candidate_app_federated_session_project_v1(
  text,bytea,bytea,uuid,integer,uuid,integer,bigint,timestamptz,timestamptz
) is 'Projects the newest verified central route context into one renewed short-lived agency-local Candidate session while preventing an older in-flight context from rolling it back.';
