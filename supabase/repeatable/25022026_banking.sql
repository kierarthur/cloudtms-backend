...
begin
  ...
  if jsonb_typeof(v_preview_decisions_json) <> 'object' then
  ...
  if jsonb_typeof(coalesce(v_preview_decisions_json->'case_resolution_states', '[]'::jsonb)) = 'array' then
    ...
    v_case_resolution_states := '[]'::jsonb;
  end if;

  if jsonb_typeof(coalesce(v_preview_decisions_json->'blocked_case_states', '[]'::jsonb)) = 'array' then
    v_blocked_case_states := coalesce(v_preview_decisions_json->'blocked_case_states', '[]'::jsonb);
  else
    v_blocked_case_states := '[]'::jsonb;
  end if;

  if jsonb_typeof(coalesce(v_preview_decisions_json->'safe_case_states', '[]'::jsonb)) = 'array' then
    v_safe_case_states := coalesce(v_preview_decisions_json->'safe_case_states', '[]'::jsonb);
  else
  ...
  begin
    perform public._imp_debug_audit(
      ...
      jsonb_build_object(
        ...
        'override_mode', coalesce(p_override_mode::text, null),
        ...
        'preview_decisions_json_keys_sample', (
          ...
          from (
            select e.key
            from jsonb_each(v_preview_decisions_json) e
            order by e.key
            limit 50
          ) k
        ),
...
begin
  ...
  loop
    ...
    select exists (
      ...
      where pis.cleared_at_utc is null
        ...
        and (
          ...
          or pis.snooze_until_date >= coalesce(v_batch.authoritative_payment_date, v_batch.pay_date)
        )
      limit 1
    )
    ...
    set
      ...
      status = case
        when v_case_type in ('PAYMENT_ADVANCE'::public.pay_finance_case_type_enum, 'MANUAL_DEBT_ADJUSTMENT'::public.pay_finance_case_type_enum, 'OVERPAYMENT'::public.pay_finance_case_type_enum)
  ...
  upd as (
    ...
    where pa.id = p.finance_case_id
      and pa.case_type in ('PAYMENT_ADVANCE'::public.pay_finance_case_type_enum, 'MANUAL_CREDIT_ADJUSTMENT'::public.pay_finance_case_type_enum)
  ...
  if v_catchup_needed then
    begin
      ...
      if coalesce((v_comm_result->>'ok')::boolean, true) = false then
        raise exception '%', coalesce(
          nullif(btrim(coalesce(v_comm_result->>'error','')), ''),
          'pay_settle_rail: catch-up worker communication queueing failed'
        );
      end if;
...
begin
  ...
  if v_use_golden = true or v_req_qty <= 1 then
    ...
    v_provider := upper(btrim(coalesce(v_batch.rail_provider_snapshot,'')));
    if v_provider = '' then
      raise exception 'pay_batch_auth_start: rail_provider_snapshot missing on batch';
    end if;
...
begin
  ...
  inp as (
    select coalesce(
      (select array_agg(distinct t0.x) from unnest(coalesce(v_ts_ids, array[]::uuid[])) as t0(x) where t0.x is not null),
      array[]::uuid[]
    ) as ts_ids
  ),
  this_items_raw as (
  ...
  with batch_finance_items as (
    select
      ...
      pbi.item_type,
      pbi.repayment_week_start,
      round(abs(coalesce(pbi.amount_ex_vat, 0)), 2)::numeric(12,2) as reserved_amount_ex
    from public.pay_batch_items pbi
    join public.pay_batch_candidates pbc
      on pbc.id = pbi.pay_batch_candidate_id
    where pbc.pay_batch_id = p_pay_batch_id
  ...
  -- MANUAL_DEBT_RECOVERY, and LOAN_REPAYMENT using the current
  -- canonical finance-case rules.
  ---------------------------------------------------------------------------
  if v_scope in ('PAYE','UMBRELLA') then
    ...
    overpay_cases as (
      select
        ...
        pa.created_at
      from public.pay_advances pa
      where pa.case_type = 'OVERPAYMENT'::public.pay_finance_case_type_enum
    ...
    cand_cap as (
      select
        ce.candidate_id,
        round(least(coalesce(sum(oc.outstanding_amount), 0), ce.earnings_before_recovery_ex), 2)::numeric(12,2) as recovery_total_ex
      from cand_earnings ce
      left join overpay_cases oc
        on oc.candidate_id = ce.candidate_id
      group by ce.candidate_id, ce.earnings_before_recovery_ex
      having round(least(coalesce(sum(oc.outstanding_amount), 0), ce.earnings_before_recovery_ex), 2) > 0
    ),
    alloc_base as (
      select
        cc.candidate_id,
        oc.finance_case_id,
        oc.outstanding_amount,
        oc.created_at,
        cc.recovery_total_ex,
        sum(oc.outstanding_amount) over (
          partition by cc.candidate_id
          order by oc.created_at, oc.finance_case_id
          rows between unbounded preceding and 1 preceding
        )::numeric(12,2) as cum_before_ex
      from cand_cap cc
      join overpay_cases oc
        on oc.candidate_id = cc.candidate_id
    )
    select
      ab.candidate_id,
    ...
    union_keys as (
      select eo.finance_case_id from expected_overpay eo
      union
    ...
    left join expected_overpay eo
      on eo.finance_case_id = uk.finance_case_id
    left join actual_overpay ao
      on ao.finance_case_id = uk.finance_case_id
    where uk.finance_case_id is not null
      and round(coalesce(eo.take_ex,0),2) <> round(coalesce(ao.taken_ex,0),2);
    ...
    cand_earnings as (
      select
        ...
        greatest(
          ...
          - coalesce((
              select teo.expected_overpayment_recovery_ex
              from pg_temp.tmp_fresh_candidate_expected_overpay teo
              where teo.candidate_id = cs.candidate_id
              limit 1
            ), 0),
          0
    ...
    cand_with_floor as (
      select
        ...
        round(
          ...
          2
        )::numeric(12,2) as max_manual_debt_recovery_ex
      from cand_earnings ce
      join public.candidates c
    ...
    debt_cases as (
      select
        ...
        pa.created_at
      from public.pay_advances pa
      where pa.case_type = 'MANUAL_DEBT_ADJUSTMENT'::public.pay_finance_case_type_enum
        and pa.status = 'ACTIVE'::public.pay_advance_status_enum
        and pa.outstanding_amount > 0
        and pa.weekly_due is not null
        and pa.weekly_due > 0
        and coalesce(pa.next_due_week_start, pa.start_week_start, v_week_start) <= v_week_start
    ...
    debt_repaid_wtd as (
      select
        pbc2.candidate_id,
        coalesce(
          pbi2.finance_case_id,
          case
            when pbi2.source_ref ~ '^advance:[0-9a-fA-F-]{36}$' then replace(pbi2.source_ref, 'advance:', '')::uuid
            else null
          end
        ) as finance_case_id,
      ...
      join public.pay_batches pb2
        on pb2.id = pbc2.pay_batch_id
      where pbi2.item_type = 'MANUAL_DEBT_RECOVERY'
        and pbi2.is_voided = false
    ...
    repayment_due as (
      select
        cwf.candidate_id,
        cwf.max_manual_debt_recovery_ex,
        dc.finance_case_id,
        dc.outstanding_amount,
        dc.weekly_due,
        dc.start_week_start,
        dc.next_due_week_start,
        dc.created_at,
        greatest(
          least(dc.weekly_due, dc.outstanding_amount) - coalesce(drw.repaid_wtd_ex, 0),
          0
        )::numeric(12,2) as remaining_due_ex
      from cand_with_floor cwf
      join debt_cases dc
        on dc.candidate_id = cwf.candidate_id
      left join debt_repaid_wtd drw
        on drw.candidate_id = cwf.candidate_id
       and drw.finance_case_id = dc.finance_case_id
      where cwf.max_manual_debt_recovery_ex > 0
        and greatest(
          least(dc.weekly_due, dc.outstanding_amount) - coalesce(drw.repaid_wtd_ex, 0),
          0
        ) > 0
    ...
    alloc_base as (
      select
        ...
        rd.finance_case_id,
        rd.remaining_due_ex,
        rd.max_manual_debt_recovery_ex,
        sum(rd.remaining_due_ex) over (
          partition by rd.candidate_id
          order by coalesce(rd.next_due_week_start, rd.start_week_start) nulls first, rd.created_at, rd.finance_case_id
          rows between unbounded preceding and 1 preceding
        )::numeric(12,2) as cum_before_ex
      from repayment_due rd
    )
    select
      ab.candidate_id,
      ab.finance_case_id,
      round(
        least(
          ab.remaining_due_ex,
          greatest(ab.max_manual_debt_recovery_ex - coalesce(ab.cum_before_ex, 0), 0)
        ),
        2
      )::numeric(12,2) as take_ex,
      32 as ord
    ...
    expected_manual_debt as (
      select
        tmd.finance_case_id,
        tmd.take_ex
      from pg_temp.tmp_fresh_expected_manual_debt_alloc tmd
    ),
    union_keys as (
      select emd.finance_case_id from expected_manual_debt emd
      union
      select amd.finance_case_id from actual_manual_debt amd
    )
    select
      'MANUAL_DEBT_RECOVERY' as key_type,
      uk.finance_case_id::text as key_value,
      round(coalesce(emd.take_ex,0),2)::numeric(12,2) as expected_ex,
      round(coalesce(amd.taken_ex,0),2)::numeric(12,2) as actual_ex,
      32 as ord
    from union_keys uk
    left join expected_manual_debt emd
      on emd.finance_case_id = uk.finance_case_id
    left join actual_manual_debt amd
      on amd.finance_case_id = uk.finance_case_id
    where uk.finance_case_id is not null
      and round(coalesce(emd.take_ex,0),2) <> round(coalesce(amd.taken_ex,0),2);
    ...
    with cand_scope as (
      select
        ...
        pni.net_amount::numeric(12,2) as paye_net_amount,
        coalesce((
          select teo.expected_overpayment_recovery_ex
          from pg_temp.tmp_fresh_candidate_expected_overpay teo
          where teo.candidate_id = pbc.candidate_id
          limit 1
        ), 0)::numeric(12,2)
        + coalesce((
          select round(sum(tmd.take_ex), 2)
          from pg_temp.tmp_fresh_expected_manual_debt_alloc tmd
          where tmd.candidate_id = pbc.candidate_id
    ...
    cand_with_floor as (
      select
        cl.candidate_id,
        cl.pay_channel,
        cl.earnings_before_payment_advance_recovery_ex,
        coalesce(pw.paid_wtd_before_ex, 0)::numeric(12,2) as paid_wtd_before_ex,
        coalesce(c.min_take_home_wtd, 0)::numeric(12,2) as floor_ex,
        round(
          greatest(
            least(
              cl.earnings_before_payment_advance_recovery_ex,
              (coalesce(pw.paid_wtd_before_ex, 0) + cl.earnings_before_payment_advance_recovery_ex) - coalesce(c.min_take_home_wtd, 0)
            ),
            0
          ),
          2
      ...
      where cl.earnings_before_payment_advance_recovery_ex > 0
    ),
    payment_advance_cases as (
      select
        pa.id as finance_case_id,
        pa.candidate_id,
        pa.outstanding_amount::numeric(12,2) as outstanding_amount,
    ...
    repayment_due as (
      select
        ...
        pac.finance_case_id,
        pac.outstanding_amount,
        pac.weekly_due,
        pac.start_week_start,
        pac.next_due_week_start,
        pac.created_at,
        greatest(
          least(pac.weekly_due, pac.outstanding_amount) - coalesce(rw.repaid_wtd_ex, 0),
          0
        )::numeric(12,2) as remaining_due_ex
      from cand_with_floor cwf
      join payment_advance_cases pac
        on pac.candidate_id = cwf.candidate_id
      left join repaid_wtd rw
        on rw.candidate_id = cwf.candidate_id
       and rw.finance_case_id = pac.finance_case_id
    ...
    alloc_base as (
      select
        ...
        rd.finance_case_id,
        rd.remaining_due_ex,
        rd.max_payment_advance_repayment_ex,
        sum(rd.remaining_due_ex) over (
          partition by rd.candidate_id
          order by coalesce(rd.next_due_week_start, rd.start_week_start) nulls first, rd.created_at, rd.finance_case_id
          rows between unbounded preceding and 1 preceding
    ...
    select
      ab.candidate_id,
      ab.finance_case_id as loan_id,
      round(
        least(
          ab.remaining_due_ex,
          greatest(ab.max_payment_advance_repayment_ex - coalesce(ab.cum_before_ex, 0), 0)
        ),
        2
      )::numeric(12,2) as take_ex,
      33 as ord
    ...
    union_keys as (
      select el.finance_case_id from expected_loans el
      union
    ...
    left join expected_loans el
      on el.finance_case_id = uk.finance_case_id
    left join actual_loans al
      on al.finance_case_id = uk.finance_case_id
    where uk.finance_case_id is not null
      and round(coalesce(el.take_ex,0),2) <> round(coalesce(al.taken_ex,0),2);
...
declare
  ...
  v_target_status public.pay_advance_status_enum;
  v_event_type text;
...
begin
  if p_finance_case_id is null then
    raise exception '%', jsonb_build_object(
      'error','PAY_FINANCE_CASE_PAUSE_RESUME',
      'code','FINANCE_CASE_ID_REQUIRED',
      'message','pay_finance_case_pause_resume: finance_case_id is required'
    )::text;
  end if;
  ...
  if p_actor_user_id is null then
    raise exception '%', jsonb_build_object(
      'error','PAY_FINANCE_CASE_PAUSE_RESUME',
      'code','ACTOR_USER_ID_REQUIRED',
      'message','pay_finance_case_pause_resume: actor_user_id is required'
    )::text;
  end if;
  ...
  if v_action not in ('PAUSE','RESUME') then
    raise exception '%', jsonb_build_object(
      'error','PAY_FINANCE_CASE_PAUSE_RESUME',
      'code','ACTION_INVALID',
      'message','pay_finance_case_pause_resume: action must be PAUSE or RESUME'
    )::text;
  end if;
  ...
  where pa.id = p_finance_case_id
    and pa.case_type in ('PAYMENT_ADVANCE'::public.pay_finance_case_type_enum, 'OVERPAYMENT'::public.pay_finance_case_type_enum, 'MANUAL_DEBT_ADJUSTMENT'::public.pay_finance_case_type_enum)
  for update;
  ...
  if v_case.id is null then
    raise exception '%', jsonb_build_object(
      ...
      'message','pay_finance_case_pause_resume: repayable finance case not found',
  ...
  if coalesce(v_case.write_off_reason,'') <> '' or v_case.written_off_at_utc is not null then
    raise exception '%', jsonb_build_object(
      ...
      'message','pay_finance_case_pause_resume: written-off finance cases cannot be paused or resumed',
      'finance_case_id', p_finance_case_id::text
    )::text;
  end if;
  ...
  if round(coalesce(v_case.outstanding_amount,0),2) <= 0 then
    raise exception '%', jsonb_build_object(
      ...
      'message','pay_finance_case_pause_resume: finance case has no outstanding balance',
      'finance_case_id', p_finance_case_id::text
    )::text;
  end if;

  if v_action = 'PAUSE' then
    if v_case.status = 'PAUSED'::public.pay_advance_status_enum then
      return jsonb_build_object(
        'ok', true,
        'finance_case_id', p_finance_case_id::text,
    ...
    v_target_status := 'PAUSED'::public.pay_advance_status_enum;
    v_event_type := 'PAUSED';
  else
    ...
    if v_case.status <> 'PAUSED'::public.pay_advance_status_enum then
      raise exception '%', jsonb_build_object(
        ...
        'message','pay_finance_case_pause_resume: only paused finance cases can be resumed',
        'finance_case_id', p_finance_case_id::text,
        'current_status', v_case.status::text
      )::text;
    end if;
    v_target_status := 'ACTIVE'::public.pay_advance_status_enum;
  ...
  values (
    ...
    case when v_action = 'PAUSE' then 'Finance case paused' else 'Finance case resumed' end
  );
...
create or replace function public.pay_finance_case_write_off(
  p_finance_case_id uuid,
  p_actor_user_id uuid,
...
declare
  ...
  v_write_off_reason text := nullif(btrim(coalesce(p_write_off_reason,'')), '');
begin
  if p_finance_case_id is null then
    raise exception '%', jsonb_build_object(
      'error','PAY_FINANCE_CASE_WRITE_OFF',
      'code','FINANCE_CASE_ID_REQUIRED',
      'message','pay_finance_case_write_off: finance_case_id is required'
    )::text;
  end if;
  ...
  if p_actor_user_id is null then
    raise exception '%', jsonb_build_object(
      'error','PAY_FINANCE_CASE_WRITE_OFF',
      'code','ACTOR_USER_ID_REQUIRED',
      'message','pay_finance_case_write_off: actor_user_id is required'
    )::text;
  end if;
  ...
  if v_write_off_reason is null then
    raise exception '%', jsonb_build_object(
      ...
      'message','pay_finance_case_write_off: write_off_reason is required'
    )::text;
  end if;
  ...
  if v_case.id is null then
    raise exception '%', jsonb_build_object(
      'error','PAY_FINANCE_CASE_WRITE_OFF',
      'code','FINANCE_CASE_NOT_FOUND',
      'message','pay_finance_case_write_off: repayable finance case not found',
      'finance_case_id', p_finance_case_id::text
    )::text;
  end if;

  if v_case.written_off_at_utc is not null or coalesce(v_case.write_off_reason,'') <> '' then
    return jsonb_build_object(
      'ok', true,
      'finance_case_id', p_finance_case_id::text,
      'case_type', v_case.case_type::text,
  ...
  v_before_json := jsonb_build_object(
    ...
    'outstanding_amount', round(coalesce(v_case.outstanding_amount,0),2),
    'next_due_week_start', case when v_case.next_due_week_start is null then null else v_case.next_due_week_start::text end,
    'written_off_at_utc', case when v_case.written_off_at_utc is null then null else v_case.written_off_at_utc::text end,
    'write_off_reason', v_case.write_off_reason
  );

  update public.pay_advance_reservations par
  set status = 'RELEASED',
      released_at_utc = coalesce(par.released_at_utc, v_now_utc),
      released_reason = 'WRITE_OFF',
      updated_by_user_id = p_actor_user_id
  where par.finance_case_id = p_finance_case_id
    and par.status in ('RESERVED','COMMITTED');
  ...
  set
    ...
    next_due_week_start = null::date,
    updated_at = v_now_utc,
    written_off_at_utc = v_now_utc,
    written_off_by_user_id = p_actor_user_id,
    write_off_reason = v_write_off_reason,
    cleared_at_utc = coalesce(pa.cleared_at_utc, v_now_utc),
  ...
  values (
    ...
    v_after_json,
    v_write_off_reason,
    'Finance case written off'
  );

  perform public._audit_insert(
    'pay_advances',
    p_finance_case_id::text,
    'FINANCE_CASE_WRITTEN_OFF',
    v_before_json,
    v_after_json,
    v_write_off_reason,
    p_actor_user_id
...
begin
  ...
  select
    ...
    count(*)::integer
  into
    v_component_remaining_amount,
    v_open_component_count
  from public.pay_finance_case_components pfc
  where pfc.finance_case_id = p_finance_case_id
    and pfc.closed_at_utc is null;

  v_remaining_amount := case
    when v_open_component_count > 0 then v_component_remaining_amount
    else round(coalesce(v_case.outstanding_amount,0),2)
  end;
  ...
  v_before_json := jsonb_build_object(
    ...
    'status', v_case.status::text,
    'outstanding_amount', round(coalesce(v_case.outstanding_amount,0),2),
    'component_remaining_amount', v_component_remaining_amount,
...
declare
  ...
  v_component_created boolean := false;
...
begin
  ...
  values (
    p_finance_case_id,
    v_component.id,
    case when v_component_created then 'COMPONENT_CREATED' else 'COMPONENT_UPDATED' end,
    v_now_utc,
    p_actor_user_id,
    null::uuid,
    v_component_before_json,
