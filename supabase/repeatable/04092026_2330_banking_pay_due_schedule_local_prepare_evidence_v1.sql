-- CloudTMS Banking Pay scheduled execution reliability.
-- Exact replacement of pay_batches_claim_due_scheduled. The only semantic
-- change is that the post-lock provider-evidence guard uses the canonical
-- transfer classifier. A transfer request_id created during local preparation
-- is an idempotency identity, not proof that a provider call occurred.
...
DECLARE
  ...
  v_input_json jsonb;
  v_operation_id uuid;
  v_authorised_operation_id uuid;
...
BEGIN
  ...
  LOOP
    ...
    SELECT CASE
             WHEN NULLIF(pg_catalog.btrim(COALESCE(auth_request.execution_intent_json->>'operation_id', '')), '')
                    ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
               THEN (auth_request.execution_intent_json->>'operation_id')::uuid
             ELSE NULL::uuid
           END
    INTO v_authorised_operation_id
    FROM public.pay_batch_auth_requests AS auth_request
    WHERE auth_request.pay_batch_id = v_batch_record.id
      AND auth_request.state = 'AUTHORISED'
    ORDER BY auth_request.created_at_utc DESC NULLS LAST, auth_request.id DESC
    LIMIT 1;
    ...
    IF EXISTS (
      ...
      FROM public.pay_bank_transfer_execution_classify(
        v_batch_record.id,
        'ALL',
        v_authorised_operation_id,
        true,
        'SCHEDULE_ACTION'
      ) AS transfer_evidence
      WHERE COALESCE(transfer_evidence.has_provider_submission_evidence, false)
         OR COALESCE(transfer_evidence.has_provider_event_evidence, false)
         OR COALESCE(transfer_evidence.has_provider_attempt_without_external_id, false)
         OR COALESCE(transfer_evidence.has_operation_submit_attempt, false)
         OR COALESCE(transfer_evidence.has_ambiguous_external_evidence, false)
         OR COALESCE(transfer_evidence.is_terminal_or_completed, false)
         OR COALESCE(transfer_evidence.has_provider_submit_blocker, false)
         OR COALESCE(transfer_evidence.is_unattempted_submit_eligible, false) IS NOT TRUE
    ) OR EXISTS (
      SELECT 1
