-- Set-wise read path for the unchanged Draft integrity policy.
...
v_stage := 'STAGE_21_BREAKDOWN_INTEGRITY_MISSING';
  ...
  IF p_operation_id IS NOT NULL THEN
    WITH selected_advanced_lines AS (
      ...
      FROM public.banking_pay_operation_candidate_scope AS scope_row
      CROSS JOIN LATERAL private.pay_workbench_operation_selected_advanced_lines_v8(p_operation_id, scope_row.id) AS line_element(value)
      WHERE scope_row.operation_id = p_operation_id
        AND scope_row.pay_batch_id = v_batch_id
  ...
  IF p_operation_id IS NOT NULL THEN
    ...
    WITH operation_scope_rows AS MATERIALIZED (
      SELECT
        scope_row.id,
      ...
      SELECT 'DRAFT_SCOPE_SELECTED_LINES_MISSING'::text AS check_code,
             ...
             jsonb_build_object(
               ...
               'selected_row_count_seeded_in_page', CASE
                 ...
                 ELSE 0
               END,
               'selected_canonical_preview_lines_count', coalesce((SELECT selected_count.line_count FROM operation_selected_line_counts AS selected_count WHERE selected_count.candidate_scope_id = scope_row.id), 0)
             ) AS detail_json
      FROM public.banking_pay_operation_candidate_scope AS scope_row
      WHERE scope_row.operation_id = p_operation_id
        AND scope_row.pay_batch_id = v_batch_id
        ...
        AND coalesce((SELECT selected_count.line_count FROM operation_selected_line_counts AS selected_count WHERE selected_count.candidate_scope_id = scope_row.id), 0) = 0
      ...
      FROM (
        ...
        FROM operation_selected_lines AS expected_channel
