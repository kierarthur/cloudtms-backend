-- Add plain-English, read-only Action Required list facts without changing
-- task ownership, payment economics, selection or Draft authority.
...
BEGIN
 ...
 WITH tasks AS MATERIALIZED (
  ...
  UNION ALL
  SELECT b.task_key,b.candidate_id,b.preview_row_id,b.affected_by_task,b.source_payload
  FROM private.pay_workbench_modal_bank_task_members_v2(s,p_options_json->>'pay_channel_scope',
   v_provider,v_environment,true) b
  UNION ALL
  SELECT x.task_key,x.candidate_id,x.preview_row_id,false,x.source_payload
  ...
  SELECT i.identity,
   ...
   CASE WHEN t.affected_payment_count_complete AND t.affected_payment_count=1
     AND count(DISTINCT m.preview_row_id) FILTER(WHERE m.affected)=1 THEN
    max(NULLIF(m.payload->>'amount_display','')) FILTER(WHERE m.affected) END AS affected_display_amount,
   CASE WHEN t.affected_payment_count_complete AND t.affected_payment_count=1
     AND count(DISTINCT m.preview_row_id) FILTER(WHERE m.affected)=1 THEN
