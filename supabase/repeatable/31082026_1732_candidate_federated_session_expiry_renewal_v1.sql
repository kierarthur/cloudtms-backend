-- Renew an exact verified control-plane projection for the full lifetime of the
-- newly signed route context. The session identity remains stable; only the
-- short-lived agency-local expiry is refreshed after every authority check.
...
comment on function public.candidate_app_federated_session_project_v1(
  text,bytea,bytea,uuid,integer,uuid,integer,bigint,timestamptz,timestamptz
) is 'Projects a freshly verified central route context into one renewed short-lived agency-local Candidate session without issuing a browser refresh credential.';
