-- Banking Pay bounded-scope V1.2.5: bounded physical economic occurrences
...
BEGIN
  ...
  IF v_family='FROZEN_SETTLED_COMPONENT' THEN
    ...
    WITH projection_members AS (
    ...
    ), finance_reservation AS (
      SELECT reservation.pay_batch_item_id,
        BOOL_OR(UPPER(BTRIM(COALESCE(reservation.status,'')))='SETTLED'
          OR reservation.settled_at_utc IS NOT NULL) AS has_settled,
        ROUND(COALESCE(SUM(ABS(COALESCE(reservation.reserved_source_amount,
          reservation.reserved_amount,0))) FILTER(WHERE UPPER(BTRIM(COALESCE(reservation.status,'')))='SETTLED'
          OR reservation.settled_at_utc IS NOT NULL),0),2) AS settled_source_amount
      ...
      SELECT item.id AS pay_batch_item_id,owner_timesheet.timesheet_id AS source_timesheet_id,
        ...
        CASE WHEN UPPER(BTRIM(COALESCE(item.frozen_component_key_type,''))) NOT IN
          ...
          OR (UPPER(BTRIM(item.frozen_component_key_type))='TS_DAY'
  ...
  WITH projection_members AS (
    ...
    SELECT state.timesheet_id AS source_timesheet_id,state.last_settled_signature,
      ...
      '30:'||p_projected_timesheet_id::text||':'||state.timesheet_id::text||':'||
