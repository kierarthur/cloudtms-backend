...
prefix = replaceBlock(prefix, buildGuardStart, buildGuardEnd, `    if build_result#>>'{source_build_canonical_diagnostics,source_rows_seen}'<>'1'
       or build_result->>'ready_preview_line_count'<>'1'
...
set linked_timesheet_id=('43000000-0000-4000-8000-'||lpad((20000+${testCase.finance_variant_ordinal})::text,12,'0'))::uuid,
    client_id='43000000-0000-4000-8000-000000000020',
    source_family_key='timesheet:43000000-0000-4000-8000-'||lpad((20000+${testCase.finance_variant_ordinal})::text,12,'0'),
    ...
    source_basis_json=jsonb_build_object(
      'timesheet_id','43000000-0000-4000-8000-'||lpad((20000+${testCase.finance_variant_ordinal})::text,12,'0'),
...
begin
  ...
  if v_review.suggestion_provenance<>'STALE_SAVED_RESOLUTION'
     or coalesce(v_review.is_reusable_saved_resolution,true)
     or not coalesce(v_review.is_stale_saved_resolution,false)
     or not coalesce(v_review.requires_resolution,false)
     or v_review.current_component_fingerprint is not distinct from v_component.resolution_fingerprint
     or v_selected_finance_count<>0
    ...
    raise exception 'H2_SAVED_RESOLUTION_FIRST_DIVERGENCE_CHANGED:%',jsonb_build_object(
      ...
      'draft_write_count',v_draft_write_count
    );
  end if;
  raise notice 'H2_V8_SAVED_RESOLUTION_REJECT=${testCase.case_id}|STALE_SAVED_RESOLUTION|0|%',jsonb_build_object(
...
for (const testCase of selectedCases) {
  let casePrefix = replaceOnce(prefix, 'set constraints all immediate;', `${scenarioSql(testCase)}\nset constraints all immediate;`, 'scenario injection');
  const sql = `${casePrefix}${suffixSql(testCase)}`;
  ...
  const marker = `H2_V8_SAVED_RESOLUTION_REJECT=${testCase.case_id}|STALE_SAVED_RESOLUTION|0|`;
  ...
  results.push({
    ...
    status: 'EXPECTED_CURRENT_OWNER_DIVERGENCE',
    current_v8_draft_output_status: 'OPEN_NOT_REACHED',
    first_divergent_boundary: 'SAVED_RESOLUTION_WRITER_TO_CANONICAL_PREVIEW_CONSUMER',
    observed_consumer_provenance: 'STALE_SAVED_RESOLUTION',
...
process.stdout.write(`${JSON.stringify({
  contract: 'BANKING_PAY_DRAFT_V8_SAVED_RESOLUTION_MATRIX_RESULT_V1',
  engine,database,
  harness_isolation: {
  ...
  source_sha256: {
    ...
    [writerPath]: sha256(writerSource),[consumerPath]: sha256(consumerSource)
  },
  case_count: results.length,expected_divergence_count: results.length,unexpected_pass_count: 0,fail_count: 0,
  current_v8_full_typed_draft_output_pass: 0,full_typed_v1_v8_parity_pass: 0,
