interface Env {
  LIVE_POSTGREST_ORIGIN: string;
}

const REST_PREFIX = "/rest/v1";
const VERSION = "live-v1";
...
async function proxy(request: Request, env: Env): Promise<Response> {
  const inbound = new URL(request.url);
  const upstream = upstreamUrl(request, env.LIVE_POSTGREST_ORIGIN);
  ...
  console.log(JSON.stringify({
    message: "LIVE PostgREST gateway",
    method: request.method,
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/health") {
      ...
      try {
        configured = new URL(env.LIVE_POSTGREST_ORIGIN).protocol === "https:";
      } catch {
        configured = false;
      }
      return json({
        ok: configured,
        service: "cloudtms-live-miget-gateway",
        version: VERSION,
    ...
    try {
      ...
      console.error(JSON.stringify({
        message: "LIVE PostgREST gateway failed",
        error: error instanceof Error ? error.message : "Unknown error",
      }));
      return json({ error: "Database gateway unavailable" }, 502);
    }
  },
