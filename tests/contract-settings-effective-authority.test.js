...
test('the historical QR refusal helper is reachable only through its guarded service caller', () => {
  ...
  assert.match(qrRefuseServiceAcl, /set search_path = pg_catalog, public, private, extensions, pg_temp/);
...
test('the Candidate bundle always finishes with the exact current routine owners', async () => {
  ...
  assert.deepEqual(closure.paths, [
    ...
    'supabase/repeatable/04092026_1500_invoice_frozen_settings_evaluation_barrier_v1.sql',
    ...
    'supabase/repeatable/04092026_1901_client_planned_override_refresh_v1.sql',
  ]);
});
