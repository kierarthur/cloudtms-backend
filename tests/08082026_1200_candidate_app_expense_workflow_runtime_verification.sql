...
begin
  ...
  if not exists(select 1 from public.timesheets_financials
     ...
     -- The one signed expense page is materialised immediately. Its unsigned
     -- Expense Summary is regenerated by the separate automatic queue.
     or (select count(*) from public.timesheet_evidence
      where timesheet_id=v_target_timesheet)<>1 then
...
begin
  ...
  if v_response->>'state'<>'FINALISED' or (v_response->>'timesheet_id')::uuid<>v_timesheet
     ...
     -- Hours plus the signed expense page materialise here; the unsigned
     -- Expense Summary follows through its independent refresh authority.
