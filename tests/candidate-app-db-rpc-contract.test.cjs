...
const refusedCardRecovery = read(
...
test('Candidate runtime gate finishes with every current authority', () => {
  ...
  assert.deepEqual(installPaths.slice(-9), [
    ...
    'supabase/repeatable/30082026_0640_candidate_weekly_paper_prepare_state_adapter_v1.sql',
    'supabase/repeatable/30082026_0714_candidate_weekly_paper_target_ready_state_v1.sql',
    'supabase/repeatable/09022026_delete_adjustment_timesheets_safely.sql',
  ...
  assert.deepEqual(suitePaths.slice(-3), [
    ...
    'supabase/verification/06092026_2358_candidate_advanced_expense_policy_verification.sql',
  ]);
});
...
test('the targeted correction pass enforces exact categories, complete economics and complete paper returns', () => {
  ...
  assert.match(latestCapabilities, /'can_edit_expenses'[\s\S]*\('EXPENSE_ONLY','COMBINED_ALLOWED','FLEXIBLE','IMPORT_HOURS'\)[\s\S]*v_role='HOURS_ONLY'[\s\S]*v_separate or v_candidate_mutation_locked/i);
  assert.match(latestCapabilities, /'requires_carrier'[\s\S]*v_role='IMPORT_HOURS'[\s\S]*v_role='HOURS_ONLY'[\s\S]*v_separate or v_candidate_mutation_locked/i);
  assert.match(latestCapabilities, /notify pgrst, 'reload schema'/i);
...
test('rejected workflows project through the replacement current version with server-owned recovery scope', () => {
  const page = definition(sql.reads, 'candidate_app_timesheet_page_v1');
  const detail = definition(sql.reads, 'candidate_app_timesheet_detail_v1');
  const primaryAction = privateDefinition(refusedCardRecovery, '_candidate_timesheet_primary_action_v1');
