...
const workbenchRows = read('supabase/repeatable/14082026_1310_timesheet_processing_status_and_authorise_authority_v1.sql');
...
test('reference and pair issues are classified before filtering and paging', () => {
  ...
  assert.match(summary, /source_rows\.client_ts_reference_required[\s\S]*source_rows\.client_pay_reference_required[\s\S]*source_rows\.client_invoice_reference_required/i);
  assert.doesNotMatch(summary, /client_reference_settings AS MATERIALIZED/i);
  assert.match(workbenchRows, /authority\.settings_json#>>'\{values,ts_reference_required\}'[\s\S]*AS client_ts_reference_required/i);
  assert.match(workbenchRows, /authority\.settings_json#>>'\{values,require_reference_to_pay\}'[\s\S]*AS client_pay_reference_required/i);
  assert.match(workbenchRows, /authority\.settings_json#>>'\{values,require_reference_to_invoice\}'[\s\S]*AS client_invoice_reference_required/i);
