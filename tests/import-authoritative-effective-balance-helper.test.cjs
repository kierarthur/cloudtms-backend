...
test('canonical member ownership fails closed on contradictory roles', () => {
  assert.match(helper, /having count\(distinct correction_id\|\|'\|'\|\|correction_kind\)>1/);
  assert.match(helper, /into v_member_role_map,v_member_role_conflict/);
  assert.match(helper, /v_scope_unprovable[\s\S]*or v_operation_evidence_conflict or v_member_role_conflict/);
});
...
test('credits negate source hours once and retain their stored signed money', () => {
  assert.match(helper, /v_component_day:=-coalesce/);
  assert.match(helper, /v_component_pay:=coalesce\(v_line\.total_pay_ex_vat,0\)/);
  assert.match(helper, /v_component_charge:=coalesce\(v_line\.total_charge_ex_vat,0\)/);
  assert.match(helper, /v_component_margin:=coalesce\(v_line\.margin_ex_vat/);
});
...
test('effective-zero source safety distinguishes full credit from paid-uninvoiced', () => {
  assert.match(helper, /v_effective_zero:=v_b_day=0[\s\S]*and v_b_margin=0/);
...
test('source safety participates in review/apply staleness fingerprinting', () => {
  assert.match(helper, /'reconciliation-v2'/);
