...
const weeklyImportSource = fs.readFileSync(
  path.resolve(
    __dirname,
    '../supabase/repeatable/15122025_weekly_import_changed_hours_phase3.sql'
  ),
  'utf8'
);
...
const applyGuard = extractFunction(lifecycleSource, 'import_review_apply_guard_v1');
const correctionTransition = extractFunction(
  lifecycleSource,
  'import_review_correction_generation_transition_v1'
);
const bucketAllocatorStart = weeklyImportSource.search(
  /create\s+or\s+replace\s+function\s+public\._wkimp_bucket_hours_from_policy\s*\(/i
);
assert.ok(bucketAllocatorStart >= 0, '_wkimp_bucket_hours_from_policy must exist');
const bucketAllocatorTail = weeklyImportSource.slice(bucketAllocatorStart);
const bucketAllocatorEnd = /\r?\n\$\$;/.exec(bucketAllocatorTail);
assert.ok(bucketAllocatorEnd, '_wkimp_bucket_hours_from_policy must have a complete body');
const bucketAllocator = bucketAllocatorTail.slice(
  0,
  bucketAllocatorEnd.index + bucketAllocatorEnd[0].length
);

test('the canonical weekly bucket allocator emits its calculated row', () => {
  assert.match(bucketAllocator, /total_hours\s*:=\s*round\([\s\S]*return next;\s*return;/i);
});

test('reconciliation validates A buckets from the frozen schedule and exact applied policy', () => {
  assert.match(
    correctionTransition,
    /cross join lateral public\._wkimp_bucket_hours_from_policy\([\s\S]*A_schedule_json,0,start_utc[\s\S]*A_schedule_json,0,end_utc[\s\S]*A_schedule_json,0,break_mins/i
  );
  assert.match(
    correctionTransition,
    /v_expected_a_total is distinct from coalesce\(\(v_unit#>>'\{A_hours,total_hours\}'\)::numeric,0\)/i
  );
  assert.match(
    correctionTransition,
    /v_frozen_a_bucket_total<>0[\s\S]*v_expected_a_day is distinct from coalesce\(\(v_unit#>>'\{A_hours,hours_day\}'\)::numeric,0\)/i
  );
  assert.match(
    correctionTransition,
    /tf\.hours_day<>v_expected_a_day[\s\S]*tf\.hours_bh<>v_expected_a_bh/i
  );
  assert.doesNotMatch(
    correctionTransition,
    /tf\.margin_ex_vat<>-coalesce\(\(v_unit#>>'\{B_financials,margin_ex_vat\}'\)::numeric,0\)/i
  );
});
...
test('terminal operation fallback re-buckets its exact schedule with the matching replacement policy', () => {
  assert.match(helper, /tf\.timesheet_id=v_terminal_positive_timesheet_id/);
  assert.match(helper, /'policy_envelope',e\.policy_unit->'policy_envelope'/);
  assert.match(
    helper,
    /tf\.policy_snapshot_json->'correction_financials_policy_envelope'[\s\S]*=v_terminal_operation->'policy_envelope'/
  );
  assert.match(
    helper,
    /v_terminal_policy_snapshot_fingerprint=v_terminal_operation_policy_fingerprint/
  );
  assert.match(
    helper,
    /public\._wkimp_bucket_hours_from_policy\([\s\S]*v_terminal_operation_schedule->0\)->>'start_utc'[\s\S]*v_terminal_operation_schedule->0\)->>'end_utc'/
  );
  assert.match(helper, /v_terminal_operation_hours:=jsonb_build_object\(/);
  assert.match(helper, /TERMINAL_OPERATION_SCHEDULE_BUCKET_DERIVATION_FAILED/);
});
...
test('fully issued generations do not remain active invoice activity because their locks persist', () => {
  assert.match(
    helper,
...
test('the helper compares B plus mutable M against authoritative A by rate bucket', () => {
  assert.match(helper, /v_financial_position_requires_amendment:=case/);
  assert.match(helper, /v_b_day\+v_m_day is distinct from coalesce\(\(v_a_hours->>'hours_day'\)::numeric,0\)/);
  assert.match(helper, /v_b_bh\+v_m_bh is distinct from coalesce\(\(v_a_hours->>'hours_bh'\)::numeric,0\)/);
  assert.match(helper, /'financial_position_requires_amendment',v_financial_position_requires_amendment/);
});

test('the catalog evaluates every existing authoritative weekly source against financial truth', () => {
  const sourceRows = catalog.slice(
    catalog.indexOf('reconciliation_source_rows as'),
    catalog.indexOf('reconciliation_inputs as')
  );
  assert.match(sourceRows, /coalesce\(f\.import_authoritative,false\) and f\.existing_shift_id is not null/);
  assert.doesNotMatch(sourceRows, /existing_shift_start_utc|existing_shift_end_utc|existing_shift_break_minutes/);
  assert.match(
    catalog,
    /when coalesce\(\(f\.reconciliation_balance->>'financial_position_requires_amendment'\)::boolean,false\)[\s\S]*then 'APPLY_AMENDMENT'/
  );
  assert.match(catalog, /then 'FINANCIAL_POSITION'::text/);
});
...
test('apply guard re-attests frozen units without an illegal full lateral join', () => {
  assert.doesNotMatch(applyGuard, /full\s+join\s+lateral/i);
  assert.match(applyGuard, /from pg_temp\.import_review_reconciliation_units_v1 frozen\s+where not exists/i);
  assert.match(applyGuard, /current_unit\.u->>'action_id'=frozen\.action_id/);
  assert.match(applyGuard, /current_unit\.u->>'unit_fingerprint'=frozen\.unit_fingerprint/);
  assert.match(applyGuard, /jsonb_array_length\(coalesce\(v_envelope->'reconciliation_units','\[\]'::jsonb\)\)/);
});

test('correction transition hashes the extracted policy envelope unambiguously', () => {
  assert.doesNotMatch(
    correctionTransition,
    /u->'operation_policy_envelope'-'envelope_fingerprint'/
  );
  assert.equal(
    (correctionTransition.match(/\(\(u->'operation_policy_envelope'\)-'envelope_fingerprint'\)/g) || []).length,
    2
  );
});

test('post-commit validation excludes only its own guarded operation from in-progress evidence', () => {
  assert.match(
    helper,
    /v_transition_operation_id uuid:=case[\s\S]*cloudtms\.import_reconciliation_operation_id/
  );
  assert.match(
    helper,
    /e\.operation_id is distinct from v_transition_operation_id/
  );
  assert.match(
    correctionTransition,
    /set_config\('cloudtms\.import_reconciliation_operation_id',p_operation_id::text,true\)/
  );
});

test('post-commit validation re-attests immutable B without rejecting the approved mutable repair', () => {
  assert.doesNotMatch(
    correctionTransition,
    /v_current_invoice_fingerprint\s+is\s+distinct\s+from\s+v_unit->>'B_invoice_fingerprint'/i
  );
  for (const frozenField of [
    'B_effective_invoice_ids',
    'B_effective_invoice_line_ids',
    'B_hours',
    'B_financials',
    'B_standard_schedule_json',
  ]) {
    assert.match(
      correctionTransition,
      new RegExp(`v_unit->'${frozenField}'`),
      `post-commit validation must re-attest ${frozenField}`
    );
  }
});
