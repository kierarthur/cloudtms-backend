...
test('current electronic Candidate evidence requires its exact durable manager approval lineage', () => {
  ...
  assert.match(handler, /candidate_submission_workflows[\s\S]*select=id,generation,state,route,manager_approved_at_utc,finalised_at_utc/);
  ...
  assert.match(handler, /const candidateManagerApprovalConfirmed = candidateSubmissionApprovalConfirmedFor/);
  assert.match(handler, /route === 'PAPER'[\s\S]*state === 'FINALISED'[\s\S]*workflow\?\.finalised_at_utc[\s\S]*generation === artifactGeneration/);
  assert.match(handler, /candidateSubmissionApprovalAtFor[\s\S]*workflow\?\.finalised_at_utc/);
  ...
  assert.match(handler, /candidateSubmissionApprovalConfirmedFor\(component\?\.workflow_id, component\?\.workflow_generation\)/);
