...
function _flushPendingWrites() {
  ...
  try {
    ...
    for (const tel of Object.keys(grouped)) {
      ...
      const toApply = []; // { r, c, value, bg, ymd, code }
      ...
      ymds.forEach(ymd => {
        ...
        toApply.push({ r: avRow.rowIndex, c: col, value: m.value, bg: m.bg, ymd, code: codeUp });
        applied++;
      });
      ...
      if (toApply.length) {
        ...
        postWriteMirrors.push({
          msisdn: tel,
          results: toApply.map(x => ({ ymd: x.ymd, applied: true, code: x.code }))
        });
      }
  ...
  postWriteMirrors.forEach(item => {
    if (typeof ctmsP3_mirrorLegacyAvailability_ === 'function') {
      ctmsP3_mirrorLegacyAvailability_(item.msisdn, [], item.results);
    }
  });
...
if (__action === "EMAILHISTORY_UPDATED") {
  ...
  return OK({ ok: true, ehRev: _getInt(K_EH_REV, 0), warmed });
    ...
    // ----- legacy: candidate availability changes (no action) -----
    {
      ...
      const __legacyAvailabilityResponse = { ok: true, results };
      if (typeof ctmsP3_mirrorLegacyAvailability_ === 'function') {
        const __appliedAvailabilityResults = results.filter(r => r.applied === true && r.deferred !== true);
        ctmsP3_mirrorLegacyAvailability_(
          msisdn,
          __appliedAvailabilityResults.map(r => ({ ymd: r.ymd, code: r.code })),
          __appliedAvailabilityResults
        );
      }
      return OK(__legacyAvailabilityResponse);
    }
  } catch (e2) {
...
const __legacyBusyTilesResponse = {
  ok: true,
  tiles: tilesWithCid,
...
return _ok(typeof ctmsP3_mergeLegacyTiles_ === 'function'
  ? ctmsP3_mergeLegacyTiles_(__legacyBusyTilesResponse, msisdn, wantYmds[0])
  : __legacyBusyTilesResponse);
...
return _ok(typeof ctmsP3_mergeLegacyTiles_ === 'function'
  ? ctmsP3_mergeLegacyTiles_(__tilesResponse, msisdn, wantYmds[0])
  : __tilesResponse);
