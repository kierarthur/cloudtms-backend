...
Exact mutation replay is governed by the durable semantic receipt described in `CANDIDATE_EXECUTION_REPLAY_AND_PAPER_FAILURE_AUTHORITY.md`. Server-generated email tokens are deterministic; finalisation receipt lookup precedes mutable lifecycle checks; and PAPER scheduler/Office retries share the exact outbox attempt lease, failure catalogue, deadline and backoff authority.
