...
Status: deployed TEST broker/private-backend authority map, updated 11 August 2026 through PAPER source-owner and claim-isolation closure at runtime commit `0349b76903057c3d01961af6dff81ea05a57576f`: the immutable current-token workflow owns route intervention, unselected nonterminal source workflows block retirement before mutation, terminal history remains immutable, and claim-level cancellation cannot invalidate another claim's live delivery.
...
PAPER retirement keeps QR source, delivery owner and economic target separate. The source and current-token owner come from exact immutable workflow-generation/mail receipts; the affected economic target comes from the locked workflow/route target and may be a different expense carrier. The database locks every waiting/received/finalised source workflow and outbox row, fences live provider permits, and invalidates the current token through its exact owner. Before any delivery mutation it proves that no unselected `AWAITING_PAPER_RETURN` or `RECEIVED` workflow would be stranded; only terminal history may be preserved. With no live token, exactly one nonterminal source workflow may own route intervention; multiple or ambiguous workflows fail closed. Cancellation, supersession, rejection and confirmed route conversion compose this owner under the same family lock and mail permit barrier. No Candidate, frontend or broker code may infer, interchange or bypass these identities.
