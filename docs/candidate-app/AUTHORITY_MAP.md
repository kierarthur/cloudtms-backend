...
Exact mutation replay is governed by the durable semantic receipt described in `CANDIDATE_EXECUTION_REPLAY_AND_PAPER_FAILURE_AUTHORITY.md`. Server-generated email tokens are deterministic; reminder/renewal bind one exact approval-request generation; finalisation receipt lookup precedes mutable lifecycle checks; and PAPER scheduler/Office retries share the exact outbox operation/attempt lease, failure catalogue, deadline and advancing backoff authority. Replayed `CLAIMED` receipts never create a second executor, and the Office retry UUID retains the complete final result rather than a transient in-progress response.
