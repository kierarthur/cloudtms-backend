...
Status: deployed TEST broker/private-backend authority map, updated 11 August 2026 through complete PAPER caller closure: retryable `RECEIVED` cancellation/supersession, `RECEIVED`/finalised W09 route intervention, shared route/rejection family locking, atomic provider-submit permission, shared-source lease/token/mail/notification fencing, exact claim-record replacement, current-version expense anchoring, generation/state-safe upload replay, deterministic immutable documents, recovery-safe paper release and exact manager methods at runtime commit `465924aa1d692e41a0fe3bbc204b6073571d40e5`.
...
PAPER retirement keeps two identities separate: the QR source comes from the exact frozen delivery mail context/hash, while the affected economic target comes from the locked workflow/route target. They may be the same hours row or different hours-anchor/expense-carrier rows. The database treats `AWAITING_PAPER_RETURN`, retryable `RECEIVED` and `FINALISED` as one closed retirement set, locks every relevant workflow and outbox row for the source, fences all live provider leases, invalidates the current token through its exact owner once, then retires every delivery surface while preserving sent/signed history and unrelated finalised workflow/economic truth. Cancellation, supersession, office rejection and confirmed route conversion compose this source-set owner; route conversion and rejection acquire one family advisory lock before any target or workflow row lock. The mail adapter obtains a service-only atomic submit permit from the same workflow RPC, and `PAPER_RETURN`/retirement transitions lock and respect that exact outbox permit. No Candidate, frontend or broker code may infer, interchange or bypass these identities.
