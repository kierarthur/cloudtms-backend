...
Status: active implementation; TEST-only. Updated: 11 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented, published and deployed to TEST with every Candidate feature flag false. Runtime commit `0349b76903057c3d01961af6dff81ea05a57576f` closes PAPER source ownership and claim isolation: route preview/confirmation select the exact immutable current-token workflow rather than historical `timesheets.candidate_workflow_id`; source-wide retirement cannot preserve an unselected `AWAITING_PAPER_RETURN` or `RECEIVED` workflow; and claim-level cancellation/supersession fails before mutation when another nonterminal claim owns the shared QR source. Clean PostgreSQL 17.6 and 18.1 installs pass all 22 runtime/concurrency suites; 119/119 focused Candidate tests and 451/451 complete backend tests pass; all three Worker builds pass. GitHub runtime workflow `31458765280` and safe migration workflow `31458765181` passed; the exact repeatables are installed; all three TEST Workers were deployed and health/isolation checked. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS frontend implementation and API freeze.
...
Backend topology closure requires source review, focused public/private boundary and upload tests, full backend regression, OpenAPI validation, builds for both Worker artefacts, exact approved commit/push, explicitly approved TEST deployment, harmless disabled-state runtime proof and independent audit. The current PAPER source-owner/claim-isolation correction is published and deployed as recorded below. Its corrected repeatable SQL definitions were installed through the repository safe-migration workflow after the disposable PostgreSQL gates passed. No Candidate workflow, Candidate email, Candidate notification, R2 business object or financial mutation was created.

Current end-to-end closure verification: runtime commit `0349b76903057c3d01961af6dff81ea05a57576f` passes 119/119 focused Candidate/backend/broker/DB tests and 451/451 complete backend tests. The full Candidate install and all 22 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1 locally and in GitHub workflow `31458765280`. Safe migration workflow `31458765181` installed the exact changed repeatables in TEST. Ledger SHA-256 matches the exact published LF-normalised source: workflow/PAPER source-owner and retirement authority `5353174d7b16cd8dafe4e74bd8234fcbafaaf4efe6ba829d3ad2440abe75e967`; route/version authority `633e8fc98fab304f9eae8ac46970eeb2515150006fc063dfeae5f233b88a6940`. Installed definition checks prove exact immutable current-token owner selection, terminal-only preserved source history, the nonterminal source-workflow postcondition, claim-level cancellation isolation, the service-only provider submit permit and the unchanged 14-RPC access boundary. Public broker, private Candidate API and normal backend builds pass. Deployed TEST versions are private API `d3fabe29-dd8e-4c04-b241-4c264b3e6ed4`, public broker `287591b1-73ec-4e79-a6b0-2f0bfbb7ff20`, and normal TEST backend `c139889f-b392-46e4-9891-9fa3130d0d13`. Broker and normal-backend health/readiness are `200/200`; broker readiness proves the private service binding. Direct public Candidate/manager routes on the normal backend return `404`. Post-deployment state remains 14/14 service-role-only Candidate RPCs, all 12 Candidate flags false, auto-authorisation false, all seven Candidate tables empty and Candidate-bound mail empty. The current-decisions PDF requires no policy amendment because this correction implements its existing claim independence, obsolete-delivery, exceptional route-intervention and immutable-history rules.
