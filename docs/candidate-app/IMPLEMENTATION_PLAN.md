...
Current paper-mail closure verification: 105/105 focused Candidate/backend/broker/DB tests and 435/435 complete backend tests pass after rebasing over the unrelated Banking Pay source already published by the other chat. The full Candidate install and all 14 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1. Public broker, private Candidate API and normal backend dry-run builds pass, and the OpenAPI 3.1 contract validates. Final GitHub Candidate DB runtime workflow `31390040324` and safe migration workflow `31390040301` completed successfully. The canonical mail-claim repeatable was moved, without duplication, to the top-level repeatable directory consumed by the safe installer; TEST ledger SHA-256 and the installed canonical definition now match the exact published Git blob. Post-deployment health/readiness checks returned `200/200` for the public broker and normal TEST backend, while direct public Candidate routing on the normal backend returned `404` as required. Deployed TEST versions are private API `31329082-50e7-4405-8d5b-960f6e16d96b`, public broker `0b4a394e-87aa-4d25-9dfb-4f6132a7a94f`, and normal TEST backend `ebcfbdc1-0ad3-4706-8c6d-a3b128edfb69`. Post-deployment TEST state remains all Candidate feature flags false, Candidate auto-authorisation false, all seven Candidate tables empty, no Candidate-bound mail and no paper-readiness notification. The current decisions PDF was rechecked as the controlling policy record and requires no policy amendment for this implementation correction.
