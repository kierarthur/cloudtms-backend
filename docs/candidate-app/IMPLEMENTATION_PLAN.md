...
Status: active implementation; TEST-only. Updated: 10 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented, published and deployed to TEST with every Candidate feature flag false. The current Candidate runtime correction is published at commit `bf96c859c38367e6c12aee7daa694086ba50b104`. PostgreSQL runtime and safe-migration GitHub gates passed, the exact corrected repeatable is installed in TEST, and the private API, public broker and normal TEST backend were redeployed from that source. Whole-record office rejection now retires a finalised workflow only where its exact target is the rejected current timesheet, while pre-finalisation workflows retain target-or-anchor rejection and a separate finalised expense workflow remains isolated from rejection of its anchor hours. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS frontend implementation and API freeze.
...
Current end-to-end closure verification: 111/111 focused Candidate/backend/broker/DB tests and 445/445 complete backend tests pass against runtime commit `bf96c859c38367e6c12aee7daa694086ba50b104`. The full Candidate install and all 18 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1. Public broker, private Candidate API and normal backend dry-run builds pass, and the OpenAPI 3.1 contract validates. GitHub Candidate DB runtime workflow `31415225026` and safe migration workflow `31415223519` completed successfully. TEST repeatable ledger SHA-256 `dc6cfcb103d5943eaadab3bfd229cc658f85f19dff20fe881acc02175d96354f` matches the exact published Git blob, and read-only installed-definition checks prove the finalised exact-target, pre-finalisation anchor, artefact-generation and captured-state guards. Post-deployment health/readiness checks returned `200/200` for the public broker and normal TEST backend, while direct public Candidate routing on the normal backend returned `404` as required. Deployed TEST versions are private API `37a7f7f9-dcb2-417f-a6e7-5c29b0e2add4`, public broker `276c54ed-0cb6-4253-b3de-34f5a0aaec92`, and normal TEST backend `57b70f56-f76c-40a8-ba6f-e2de116cbc8a`. Post-deployment TEST state remains all Candidate feature flags false, Candidate auto-authorisation false, all seven Candidate tables empty and all fourteen Candidate business RPCs installed. The 25-page current-decisions PDF was rendered and rechecked; it requires no amendment because this correction implements the existing whole-record rejection and expense-isolation policy.
