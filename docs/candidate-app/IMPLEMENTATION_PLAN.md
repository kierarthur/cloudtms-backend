...
Status: active implementation; TEST-only. Updated: 10 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented, published and deployed to TEST with every Candidate feature flag false. The latest paper-pack mail-authority correction is published at runtime commit `aec2ea4dc25286fca271fedfc427a7b4533541a1`; both PostgreSQL runtime and safe-migration GitHub gates passed, the corrected repeatables are installed in TEST, and the private API, public broker and normal TEST backend were redeployed from that published source. The correction atomically holds and binds the Candidate PAPER email before ordinary QR rendering, makes the private scheduler the only complete-pack release owner, and prevents the ordinary invoice-document completion and mail claimant from bypassing that authority. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS frontend implementation and API freeze.
...
Backend topology closure requires source review, focused public/private boundary and upload tests, full backend regression, OpenAPI validation, builds for both Worker artefacts, exact approved commit/push, explicitly approved TEST deployment, harmless disabled-state runtime proof and independent audit. The 10 August paper-mail authority correction is published and deployed as recorded below. Its three repeatable SQL definitions were installed through the repository safe-migration workflow after the disposable PostgreSQL gates passed. No Candidate workflow, Candidate email, Candidate notification, R2 object or financial mutation was created.

Current paper-mail closure verification: 59/59 focused changed-path tests; 429/429 complete backend tests after rebasing over the current unrelated Banking Pay source; 41/41 Candidate DB/RPC plus canonical DAILY structural tests; the full Candidate install and all 14 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and again on PostgreSQL 18.1; public broker, private Candidate API and normal backend dry-run builds pass. GitHub Candidate DB runtime workflow `31381619545` and safe migration workflow `31381619547` completed successfully. TEST runtime verification proves the three installed definitions, their owner/security/search-path/ACL boundaries, held Candidate-mail exclusion, ordinary non-Candidate QR behaviour and exact complete-pack claim gate. Post-deployment health checks returned `200` for the public broker (including its private service-bound probe) and normal TEST backend, while direct public Candidate routing on the normal backend returned `404` as required. Deployed TEST versions are private API `c9f0231c-ec24-47d6-a6d3-e7c0f7085b8d`, public broker `1665eaae-466b-43dc-9c5c-3448bf0e41ac`, and normal TEST backend `4d5c3254-89f0-4ebd-9530-799dee895b66`. Post-test TEST state remains all Candidate feature flags false, all seven Candidate tables empty and no transactional verification fixture rows retained. The current decisions PDF was rechecked as the controlling policy record and remains fully consistent with the correction.
