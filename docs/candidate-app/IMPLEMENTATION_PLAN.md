...
Status: active implementation; TEST-only. Updated: 11 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented and deployed to TEST with every Candidate feature flag false. This revision completes the approved manager action contract and the Candidate timesheet-detail action hub: provider acceptance—not enqueue time—owns manager-send timing; a reminder remains a same-request reminder; renewal is allowed only for an expired unchanged request; cancellation requires a reason, retires delivery authority and sends a deterministic withdrawal where provider acceptance is proved; and detail returns the exact server-owned actions and manager status needed by clients. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS office-frontend implementation and API freeze.
...
Current closure verification: the complete backend suite passes 460/460 tests and the focused Candidate/backend/broker/DB/PAPER suite passes 124/124. The full Candidate install and all 27 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1. GitHub runtime workflow `31495500205` independently passed the same PostgreSQL matrix, and safe TEST migration workflow `31495500073` installed runtime commit `a14b60b734560fc5ddf0109bbef6e21eb46e4857`. OpenAPI validation and all three Worker dry-run builds pass. The installed repeatable ledger matches the two corrected LF-normalised source hashes (`30cfbe23762d92af40a40c4e0cb4e6085ecb4cf631a2f48c0f9f8b6ea0c1babd` and `8eb1bf2762462164569b03d1964d99700055e7ebd7895250d5dc0964a686c2d3`), all 12 Candidate flags remain false, all seven Candidate business tables and Candidate-bound mail remain empty, and the 14 service-role-only public business RPC boundary remains exact. Active TEST Worker versions are normal backend `14554299-dcb6-4102-8ef9-7316d34b3654`, private Candidate API `407596b9-5dd4-48fa-939a-4a9b2885ef74` and public Candidate broker `7dfc34ec-b78f-48ca-8b0d-ebd847cfcebc`. Sections 4 and 27 of the current-decisions PDF remain controlling: the Current/History DB/RPC/backend contract and detail action hub are implemented, while office-frontend rendering remains deferred until independent GO.
