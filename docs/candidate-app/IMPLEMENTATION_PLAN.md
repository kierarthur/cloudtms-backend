...
Status: active implementation; TEST-only. Updated: 11 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented, published and deployed to TEST with every Candidate feature flag false. Candidate runtime commit `1b654d72b4529c32e019956320ff50ca451db466` implements the approved office rule for one incomplete standalone expense claim during an ELECTRONIC or QR/PAPER switch to MANUAL: preview returns the exact removal warning, No is zero mutation, and confirmed Yes atomically supersedes the incomplete claim (including amendable `REFUSED` while retaining refusal history), retires any obsolete delivery authority, closes a live manager request where applicable, and only then rotates the timesheet to MANUAL. Ambiguous, multiple, unrelated, protected-history and provider-handoff cases remain fail-closed. Clean PostgreSQL 17.6 and 18.1 installs pass all 24 runtime/concurrency suites; 126/126 focused Candidate evidence, 60/60 Candidate boundary tests, 38/38 DB/RPC structural tests and 451/451 complete backend tests pass. GitHub runtime workflow `31479385933` and safe migration workflow `31479385936` passed; the exact repeatables are installed. TEST Worker versions are private API `da9a8487-5623-42e8-8c33-5e1cddb960c9`, public broker `99869508-3955-4318-916f-2c243047fdea`, and normal TEST backend `16357aa3-598f-48c5-973c-b1466af7f49e`. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS office-frontend implementation and API freeze.
...
### Mandatory next DB/RPC/backend planning item — Current and History

Section 4 of `CloudTMS_Candidate_App_Current_Decisions.pdf` now controls the Timesheets-page partition. This runtime does **not** implement that later read-contract amendment. Before any CloudTMS office-frontend code is written, the independent handover reviewer must provide a highly detailed DB/RPC/backend implementation plan that identifies the exact existing read owners, response fields, pagination/cursor effects, indexes, ordering, cutoff calculations, compatibility approach and regression matrix.

The plan must preserve these non-overlapping server rules:
...
The planning response must remain in DB/RPC/backend scope. It must not implement the CloudTMS office frontend, public broker, responsive Candidate web, iOS or Android client. No eighth Candidate table or fifteenth public Candidate business RPC is assumed or authorised.
...
Current end-to-end closure verification: runtime commit `1b654d72b4529c32e019956320ff50ca451db466` passes 126/126 focused Candidate evidence, 60/60 Candidate boundary tests, 38/38 DB/RPC structural tests and 451/451 complete backend tests. The full Candidate install and all 24 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1 locally and in GitHub workflow `31479385933`. Safe migration workflow `31479385936` installed the exact changed repeatables in TEST. Ledger SHA-256 matches the exact published LF-normalised source: workflow/PAPER source-owner authority `1b42091f1ba8a72e6d55be36b5f63d1675b1121012d06ce81c9f3a904b52169a`; route/version authority `8db97f6a2c310bb49c5a77c46202cf9f45d2d178855b3dda232b7e7417fa9372`. Installed definition checks prove the exact warning, confirmed atomic incomplete-claim removal, amendable `REFUSED` source protection and unchanged 14-RPC access boundary. Active versions are private API `da9a8487-5623-42e8-8c33-5e1cddb960c9`, public broker `99869508-3955-4318-916f-2c243047fdea`, and normal TEST backend `16357aa3-598f-48c5-973c-b1466af7f49e`. Broker health is `200` and proves the private service binding; direct public Candidate routes on the normal backend return `404`. Post-deployment state remains 14/14 service-role-only Candidate RPCs, all 12 Candidate flags false, auto-authorisation false, all seven Candidate tables empty and Candidate-bound mail empty. Section 27 of the current-decisions PDF is the controlling approved product rule for this correction; the Current/History decisions in section 4 remain mandatory unimplemented work for the next detailed DB/RPC/backend and later office-frontend plans.
