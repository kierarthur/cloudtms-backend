...
Status: active implementation; TEST-only. Updated: 11 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented and deployed to TEST with every Candidate feature flag false. This revision adds the approved server-owned Current/History Timesheets contract and closes manager EMAIL lifecycle authority: invitations, reminders, renewals and withdrawals are bound to one approval request and workflow generation; invalidation retires queued/failed delivery state, fences a live provider lease, preserves accepted history, and sends a withdrawal only when provider acceptance is proved. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS office-frontend implementation and API freeze.
