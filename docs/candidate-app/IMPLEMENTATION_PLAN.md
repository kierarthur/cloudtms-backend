...
R8 implements the accepted twelve-table/thirteen-RPC Phase 2 authority and the complete Phase 1B broker-to-RPC mapping. The database owns source linkage, generations, day facts, availability, command/batch receipts, projection leases/cursors, mode transitions and external-effect receipts. The private Worker owns HMAC verification and RPC composition; the public broker rebuilds strict allowlisted responses. Signed-system continuity does not consult `candidate_daily_enabled`; Candidate-facing Daily routes require that flag plus exact entitlement, authority mode, a complete current fourteen-day generation and freshness. TEST remains disabled and empty.

R8 does not edit either Google Apps Script project. Phase 3 still adds the smallest server-side compatibility adapter to Availability Apps Script and the Master Rota publisher after fresh effective-source checks. The legacy browser remains unchanged. Availability, Emergency, Master Rota publication and retained specialist services continue during coexistence and after legacy-browser retirement until separately migrated and accepted.

### 16 August 2026 Candidate Daily R7 checkpoint

The accepted Phase 0 R5 Daily/Availability specification has received independent GO. The authorised read-only Google Evidence Gate is complete. The independent R6 review accepted the Phase 1A architecture but identified nine bounded transport defects. R7 closes those defects without changing the frozen R5 operation/status/error matrix: closed typed public errors with fixed messages, route-aware response reconstruction, exact public 403 handling, source-IP plus accepted-key/shared-invalid-key pre-auth throttling, server-consumption-age nonce retention, valid signed correlation enforcement, exact declared/actual body framing, explicit ASCII/code-unit query ordering and an honest Fetch-observable header boundary. The source still contains exactly 24 dark Daily routes plus the additive disabled bootstrap capability. No Daily SQL/RPC, Candidate UI or Google adapter has been implemented.

Current next sequence is fixed: independent R8 review; Phase 3 minimal server-side Google coexistence adapter; Phase 4 Candidate Daily UI and shadow parity; controlled TEST cutover; full specialist/workflow acceptance; gradual rollout and separately authorised legacy retirement. `docs/candidate-app/CANDIDATE_DAILY_PHASE2_PHASE1B_IMPLEMENTATION_AUTHORITY.md`, `docs/candidate-app/GOOGLE_EVIDENCE_GATE_20260816.md`, the R8 handover and the complete decision matrices are the current Daily implementation/status authority. The temporary legacy browser must not be modernised: preserve its UI/login/`msisdn` behaviour and contain it behind the later narrow Apps Script-to-CloudTMS signed adapter. Retiring that browser later does not retire the Availability Sheet/Apps Script service, Emergency functions, Master Rota publication, signed synchronisation, projections, freshness checks or specialist consumers; those remain until separately migrated and accepted.
...
- public access/refresh values are broker-encrypted audience/environment-bound envelopes; exact private login, activation and refresh receipts are represented by stable opaque public session IDs and byte-stable deterministic v4 public credentials whose frozen issuing-key versions are authenticated by HMAC identity, key derivation and AES-GCM authenticated data, while explicitly retained v1/v2/v3 readers support the bounded rollout; the broker proves every selected wrapping secret before allowing the private session mutation to start, and internal Candidate tokens are never returned to the client;
...
- Timesheet Summary has a dedicated read-only **Candidate Submission** column immediately after **Processing Status** and before **Issues** by default. It contains no actions. It behaves like the other user-configurable Summary columns: drag/reorder, resize and label-based sorting are supported and persisted; after the user changes its position, that saved order is authoritative. Candidate state never becomes an Issue. Candidate projection truth is embedded in the initial authenticated Summary response and paints with the normal row rather than hydrating through a later browser fan-out.
...
Every factual Candidate authentication, account and session mutation requires one caller-owned bounded `idempotency_key`: challenge start/resend/verify, password activation/reset completion, login (including existing-account lockout mutation and generic unknown-account failure), refresh, logout, TEST candidate selection, notification preferences, notification read acknowledgement, push-token registration and password change. The backend never substitutes a random operation key. The Candidate OpenAPI requires that key on every listed request.
...
The public boundary uses a closed unauthenticated-route catalogue; logout always unwraps the public access token and calls the private receipt owner with the exact internal bearer. Challenge start/resend validates the caller key and preserves idempotency outcomes while enumeration-masking only eligibility. Too-soon and exhausted-allowance resends are durable public 429 results; exact replay returns the same throttle, a later attempt uses a new key, and the receipt freezes the exact challenge-token issuing version for retained-reader replay across rotation and rollback. Access, refresh and PHONE credentials use key-versioned v4 envelopes whose version label is cryptographically authenticated, with explicit reader catalogues and private-result-frozen writer versions. Refresh retains the initiating public-session mapping version. Push storage encryption and semantic identity are independently versioned, and overlapping identity proofs let the private receipt select its originally frozen semantic version after key rotation. Approved rollback means the same v4-capable broker build with the former writer versions restored and all newly issued versions still readable; a pre-v4 writer broker is not an approved rollback after v4 issuance begins.
...
## 13 August 2026 mixed-version concurrency closure

Private Worker rolling deployment is now part of the authentication replay contract. Challenge START/RESEND treats each local token as a proposal only: the canonical database result returns the frozen winning token hash/version, and all mail reconstruction after the RPC uses those winner facts. Authentication/account actions first reserve one request-HMAC version under the durable environment/key authority and only then calculate secret proofs and factual request hashes. This covers activation, login, refresh, logout, selection, preferences, notification acknowledgement, push registration and password change without adding a table or public RPC.

Mandatory regression includes START and RESEND with writer versions 1/2, either database winner and either mail arrival order; login success/failure, activation, refresh and representative authenticated mutations across overlapping request-HMAC writers; changed facts/action reuse; reader retirement; PostgreSQL 17.6/18.1 parity; and the real private-handler/PostgreSQL chain. Candidate flags remain false and no shared TEST Candidate business row or mail is created by harmless deployment verification.

## 13 August 2026 PHONE winner and refresh security-receipt closure

PHONE first execution now follows the same canonical-winner rule as challenge delivery. A private Worker may propose a deterministic manager token, but after `SELECT_PHONE_APPROVAL` returns it must use the internal approval-token hash and handoff-token key version frozen by the database receipt. The Worker reconstructs and verifies that exact winner, discards its local proposal and removes the internal hash before returning the private/public handoff. This applies when concurrent Workers use different current manager-token writer versions as well as on sequential replay.

Refresh-token-reuse is a state-changing security operation with its own durable negative outcome. Family-wide revocation and `CANDIDATE_REFRESH_TOKEN_REUSE` commit through the existing authentication receipt in one transaction. The private replay path returns that stored negative result before inspecting the revoked predecessor and never derives a successor credential for it. Exact lost-response and concurrent same-key security-event retries therefore remain stable.

Mandatory regression adds both possible PHONE database winners under writer versions 1/2, internal hash non-disclosure, PostgreSQL winner-hash replay, sequential refresh security lost-response replay and two-session concurrent refresh-reuse replay. This closure adds no Candidate table, no public Candidate RPC, no frontend wiring and no financial, invoice, payment, Banking Pay or Policy X authority.

## 13 August 2026 account-session invalidation concurrency closure

Every Candidate session create, rotate or account/family-wide invalidate operation now shares one short transaction-scoped account-session advisory lock. The stable order is authentication receipt/key ownership, account-session lock, account row, session rows, mutation and durable result. A session-to-account lookup performed before that lock supplies identity only; all state, environment, account status, session status, refresh proof, expiry and family facts are re-read authoritatively after the lock is held.

The shared boundary covers normal refresh, rotated-token reuse, existing-account password activation/reset, authenticated password change, `REVOKE_SESSIONS`, `LOCK`, `DISABLE` and `LOGIN_SUCCESS` session creation/cap enforcement. Rotated-token reuse may report `family_revoked=true` only after proving that no `ACTIVE` or `ROTATED` family row remains. Reset leaves only its new session active; password change leaves only the policy-retained initiating session active; account revoke/lock/disable leave no active session; login leaves between one and five active sessions.

The executable regression suite uses two independent PostgreSQL transactions and both operation orders for security-event reuse versus current-successor refresh, password change/reset versus another-session refresh, revoke/lock/disable versus refresh and login versus disable. PostgreSQL 17.6 and 18.1 must both pass with no deadlock or escaped successor. Candidate feature flags remain false during deployment verification. This correction adds no Candidate business table, no public Candidate RPC, no frontend wiring and no financial, invoice, payment, Banking Pay or Policy X authority.
