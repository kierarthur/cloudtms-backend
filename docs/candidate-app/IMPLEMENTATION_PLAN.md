...
Current next sequence is fixed: independent R7 review; Phase 2 additive Daily SQL/RPC authoring and disposable PostgreSQL verification; separate authority for TEST installation; Phase 1B broker-to-RPC integration; Phase 3 minimal server-side Google coexistence adapter; Phase 4 Candidate Daily UI; controlled cutover and specialist acceptance. `docs/candidate-app/CANDIDATE_DAILY_PHASE1A_IMPLEMENTATION_AUTHORITY.md`, `docs/candidate-app/GOOGLE_EVIDENCE_GATE_20260816.md`, the R7 handover and the decision matrix are the current Daily implementation/status authority. The temporary legacy browser must not be modernised: preserve its UI/login/`msisdn` behaviour and contain it behind the later narrow Apps Script-to-CloudTMS signed adapter. Retiring that browser later does not retire the Availability Sheet/Apps Script service, Emergency functions, Master Rota publication, signed synchronisation, projections, freshness checks or specialist consumers; those remain until separately migrated and accepted.
