...
Current closure verification: runtime commit `d63bf708cdaf8de5124b0199b2b7eb79e4d13e6b` adds durable rejected-source lineage and makes rejected resubmission plus generic creation atomically request-aware. It passes the complete backend suite at 461/461, the focused Candidate/broker suite at 64/64 and the DB/RPC structural suite at 41/41. The full Candidate install, repeatable reinstall and all 29 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1. GitHub runtime workflow `31512287233` independently passed both PostgreSQL jobs, and safe TEST migration workflow `31512287254` installed the same runtime. OpenAPI inventory, JavaScript syntax and all three Worker builds pass. Installed LF-normalised repeatable hashes are read/action `59e24b5a71170b77fd4f336d1aa0005cf50dedc8345e22ecaedf1bf1829369dd` and workflow `c171c87f2bc4a524a9586df38925e48f261f5e6a8aca547ec386369ab374c3cb`. The replacement-lineage column, self-FK and partial unique index are installed; all 12 Candidate flags remain false, all seven Candidate business tables and Candidate-bound mail remain empty, and the 14 service-role-only public business RPC boundary remains exact. Active TEST Worker versions are normal backend `93f7e806-a204-4532-9b84-902b8ace0fd4`, private Candidate API `62a9d4f8-8c9f-4a66-b59d-b96df0c49e2e` and public Candidate broker `de619daf-5f08-407b-bf52-74bfdd80b0f0`. Normal health/readiness are 200/200, direct public Candidate routing on the normal backend is 404, and broker health/readiness are 200/200. Sections 4 and 27 of the current-decisions PDF remain controlling: the Current/History DB/RPC/backend contract and detail action hub are implemented, while office-frontend code remains deferred until independent GO and the complete frontend plan remains a separate mandatory deliverable.
...
The final execution-boundary closure additionally binds reminder/renewal to the exact request generation returned in the action envelope, excludes regenerated official presentation from `WORKER_SUBMIT` identity, derives deterministic provisional expense targets, distinguishes a fresh PAPER claim from replay of a prior `CLAIMED` receipt, advances the attempt count/backoff for timeout failures, exposes workflow-level failures before an outbox exists, and stores the complete final result of an Office retry UUID. Candidate and Office consume the same PAPER state vocabulary: `PREPARING`, `BACKOFF`, `FAILED_RETRYABLE`, `FAILED_TERMINAL`, `READY`, `RETIRED` and `STALE`.
