...
Status: active implementation; TEST-only. Updated: 10 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented and deployed to TEST with every Candidate feature flag false. The current isolated correction makes rejection projection monotonic across replacement `FINALISED`/`REFUSED` states, resolves historical expense anchors through the exact current booking/version family, returns every independently actionable hours/expense rejection scope, and invalidates a finalised separate-expense PAPER QR token through the immutable mail receipt's hours source rather than the workflow's later expense-carrier target. Clean PostgreSQL 17.6 and 18.1 installs pass all 19 runtime/concurrency suites; 112/112 focused Candidate tests and 445/445 complete backend tests pass; all three Worker dry-run builds pass. Publication, safe TEST repeatable installation, three-Worker TEST deployment and independent re-audit remain the Stage 2 gates before CloudTMS frontend implementation and API freeze.
...
Current local closure verification: 112/112 focused Candidate/backend/broker/DB tests and 445/445 complete backend tests pass. The full Candidate install and all 19 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1. Public broker, private Candidate API and normal backend dry-run builds pass, and the OpenAPI path/method contract remains covered by the focused suite. GitHub publication, safe-migration installation, installed hash/ACL/dormant-state proof, three-Worker TEST deployment, runtime health/isolation checks and final deployment identities are still pending at this point in the working plan and must replace this paragraph with exact evidence before handover. The current-decisions PDF requires no policy amendment because this correction implements its existing monotonic-current-lifecycle, independent-expense-overlay, immutable-history and obsolete-PAPER-token rules.
