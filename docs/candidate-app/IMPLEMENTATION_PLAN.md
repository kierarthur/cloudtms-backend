...
Status: active implementation; TEST-only. Updated: 11 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented, published and deployed to TEST with every Candidate feature flag false. Candidate runtime commit `7c087bc79a010944cefc4b605dcab6ea335b6c85`, with fixture-only verifier commit `a33a25d954df3632300a65b022f10e6c255a86b4`, closes the pre-delivery PAPER route seam: source context now catalogues every nonterminal PAPER workflow affected by source rotation even before a mail receipt exists, and source-rotating QR actions fail closed with zero mutation when a distinct `WORKER_DRAFT`, `WORKER_SUBMITTED` or approval-stage workflow would be stranded. Clean PostgreSQL 17.6 and 18.1 installs pass all 23 runtime/concurrency suites; 119/119 focused Candidate tests and 451/451 complete backend tests pass. GitHub runtime workflow `31469931675` and safe migration workflow `31469708425` passed; the exact repeatables are installed; unchanged Candidate Worker versions and the latest normal TEST Worker version were safely reasserted and health/isolation checked. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS frontend implementation and API freeze.
...
Backend topology closure requires source review, focused public/private boundary and upload tests, full backend regression, OpenAPI validation, builds for both Worker artefacts when Worker source changes, exact approved commit/push, explicitly approved TEST deployment, harmless disabled-state runtime proof and independent audit. The current pre-delivery PAPER route correction is published and installed as recorded below. Its corrected repeatable SQL definitions were installed through the repository safe-migration workflow after the disposable PostgreSQL gates passed. Candidate Worker source/configuration did not change in this correction; the active TEST versions were reasserted rather than rebuilt. No Candidate workflow, Candidate email, Candidate notification, R2 business object or financial mutation was created.

Current end-to-end closure verification: runtime commit `7c087bc79a010944cefc4b605dcab6ea335b6c85` and verifier commit `a33a25d954df3632300a65b022f10e6c255a86b4` pass 119/119 focused Candidate/backend/broker/DB tests and 451/451 complete backend tests. The full Candidate install and all 23 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1 in GitHub workflow `31469931675`. Safe migration workflow `31469708425` installed the exact changed repeatables in TEST. The first PostgreSQL verifier run `31469708524` failed only because the new fixture used an invalid test-only QR status; verifier commit `a33a25d...` corrected that fixture without changing runtime SQL, after which both database jobs passed. Ledger SHA-256 matches the exact published LF-normalised source: workflow/PAPER source-owner authority `439046246bf8c9c93dbb1a22e890fa7be50c914190184ecb3ad317a88fbb04db`; route/version authority `2bc42b60007732f88e27ad96a46411783af8eb0b1c1c0a962dde9491a8fc405a`. Installed definition checks prove the mail-independent affected-workflow catalogue, controlled pre-delivery shared-source conflict, locked final pre-rotation invariant, exact immutable current-token owner selection, service-only provider submit permit and unchanged 14-RPC access boundary. Candidate Worker source/configuration is unchanged from the previously built revision; active versions are private API `d3fabe29-dd8e-4c04-b241-4c264b3e6ed4`, public broker `287591b1-73ec-4e79-a6b0-2f0bfbb7ff20`, and latest normal TEST backend `40b25d1c-0eaa-4932-9372-9d6313bb5bcd`. Broker and normal-backend health/readiness are `200/200`; broker readiness proves the private service binding. Direct public Candidate/manager routes on the normal backend return `404`. Post-deployment state remains 14/14 service-role-only Candidate RPCs, all 12 Candidate flags false, auto-authorisation false, all seven Candidate tables empty and Candidate-bound mail empty. The current-decisions PDF requires no policy amendment because the correction implements its existing claim independence, exceptional route-intervention and server-owned fail-closed lifecycle rules.
