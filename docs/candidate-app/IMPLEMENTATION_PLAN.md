...
Status: active implementation; TEST-only. Updated: 10 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented, published and deployed to TEST with every Candidate feature flag false. Runtime commit `802ec3ab5cc348a86163fe0f8aab9dceca1620de` makes rejection projection monotonic across replacement `FINALISED`/`REFUSED` states, resolves historical expense anchors through the exact current booking/version family, returns every independently actionable hours/expense rejection scope, and invalidates a finalised separate-expense PAPER QR token through the immutable mail receipt's hours source rather than the workflow's later expense-carrier target. Clean PostgreSQL 17.6 and 18.1 installs pass all 19 runtime/concurrency suites; 112/112 focused Candidate tests and 445/445 complete backend tests pass; all three Worker builds pass. GitHub runtime workflow `31429293621` and safe migration workflow `31429294986` passed; the exact repeatables are installed; all three TEST Workers were deployed and health/isolation checked. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS frontend implementation and API freeze.
...
- a rejected workflow is actionable only while no later workflow of the same logical claim family—same candidate, contract, week and hours/expense family—has reached a current draft, submission, approval, received, finalised or refused lifecycle;
...
- all delivery rows must resolve to one hours-side QR source and one valid token hash whenever a live token remains. An active provider lease blocks before mutation, a mismatched source/hash fails atomically, and a missing current token is accepted only as a proved already-invalidated state;
...
The runtime matrix covers replacement `WORKER_DRAFT → FINALISED → REFUSED`, simultaneous hours/expense rejection scopes, H1→H2 expense-overlay continuity, finalised separate-expense PAPER with queued/leased/sent mail, distinct H1 QR source and E1 rejected target, replay and protected lifecycle gates. No schema, table, public RPC, public route or product policy was added. No DAILY/WEEKLY calculation, rate, pay, charge, VAT, ERNI, margin, TSFIN, Process, Authorise, invoice, payment, Banking Pay or Policy X authority changed.
...
Current end-to-end closure verification: runtime commit `802ec3ab5cc348a86163fe0f8aab9dceca1620de` passes 112/112 focused Candidate/backend/broker/DB tests and 445/445 complete backend tests. The full Candidate install and all 19 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1 locally and in GitHub workflow `31429293621`. Safe migration workflow `31429294986` installed the exact changed repeatables in TEST. Ledger SHA-256 matches the exact published Git bytes: read projection `def74b7960753062b8f6e8e047949c29de6286cc38adaeedfe988fdd95561698`, workflow/PAPER retirement `f74f67f664d49278cce82217514fc231950bf9d464889565f1da67260dff1e3c`, and rejection authority `ca592eeb23ccd4bd3242b272ac36b7c9f3ed2b7a45696d2e60819947fdf0150f`. Installed definition checks prove current-version resolution, actionable scoped rejections, mail-context QR ownership, separate rejected-target identity and mandatory invalidation receipt. Public broker, private Candidate API and normal backend builds pass. Deployed TEST versions are private API `4e51efc9-bfd9-459c-88a2-6d05eb0eb2a9`, public broker `d95ac8a1-fc09-4617-8ec4-1926e6526128`, and normal TEST backend `d2322be8-99be-4517-9b06-bf2df376f7bf`. Broker and normal-backend health/readiness are `200/200`; direct public Candidate routing on the normal backend is `404`. Post-deployment state remains 14/14 service-role-only Candidate RPCs, all Candidate flags false, auto-authorisation false, all seven Candidate tables empty and Candidate-bound mail empty. The 25-page current-decisions PDF was rendered and inspected without clipping, overlap or unreadable content; it requires no policy amendment because this correction implements its existing monotonic-current-lifecycle, independent-expense-overlay, immutable-history and obsolete-PAPER-token rules.
