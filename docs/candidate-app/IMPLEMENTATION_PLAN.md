...
Status: active implementation; TEST-only. Updated: 10 August 2026. The DB/RPC authority is installed. The separate public Candidate broker and private CloudTMS Candidate API are implemented, published and deployed to TEST with every Candidate feature flag false. The current Candidate runtime correction is published at commit `0b847735160ba8b67ca3de87911ff8555c865330`, with its structural-verifier alignment at `9361cfe34d72308530f899a7c34a124aa8c7f569`. PostgreSQL runtime and safe-migration GitHub gates passed, the exact corrected repeatables are installed in TEST, and the private API, public broker and normal TEST backend were redeployed from that source. Rejected finalised workflows now project through the replacement current version with server-owned reason, scope and resubmission action; an active replacement takes precedence while immutable rejection history remains in detail. Finalised PAPER rejection now fences and retires the preceding delivery generation before QR refusal/version rotation, blocks an active provider lease before mutation, preserves sent mail as immutable history and remains idempotent. Independent re-audit remains the Stage 2 sign-off gate before CloudTMS frontend implementation and API freeze.
...
- version-aware rejected-workflow projection through the replacement current contract-week row, with distinct hours/combined/expense recovery actions and active-replacement precedence;
- one PAPER retirement seam for both active and finalised workflows, deriving the preceding immutable delivery generation after finalisation and fencing mail leases before rejection mutation.
...
Current end-to-end closure verification: 112/112 focused Candidate/backend/broker/DB tests and 445/445 complete backend tests pass against runtime commit `0b847735160ba8b67ca3de87911ff8555c865330`. The full Candidate install and all 19 SQL runtime/concurrency suites pass on clean disposable PostgreSQL 17.6 and PostgreSQL 18.1. Public broker, private Candidate API and normal backend dry-run builds pass, and the OpenAPI 3.1 contract validates. GitHub Candidate DB runtime workflow `31422972982` and safe migration workflow `31423092836` completed successfully. TEST repeatable ledger SHA-256 values for the four amended owners match their exact published Git blobs: read projection `b1ab2206520cdf952661a3598581c083183723b0fe170871c07918441ea0a0e5`, workflow/PAPER retirement `bedd4cf0eba56288074158ec447ade4b1f2f0bca559b2f473f30c44b4f93a4ce`, rejection authority `86530c2071cf0ac05b107da5aadf06272e71edd015f6f62ea3d6e6820848feb5`, and QR refusal/settings authority `f99175195c8a67a47dab3e2f2d47c12ef0b044282372b0226f8c250a3ed0cf1a`. Read-only installed-definition checks prove version-aware rejection projection, active-replacement precedence, finalised PAPER delivery-generation retirement, finalised rejection composition and the qualified QR evidence update. Post-deployment health/readiness checks returned `200/200` for the public broker and normal TEST backend, while direct public Candidate routing on the normal backend returned `404` as required. Deployed TEST versions are private API `5cdf47c5-87aa-4166-be51-225db68a2f87`, public broker `e677ec46-1068-483c-96a4-49d63dbf5771`, and normal TEST backend `89f4a37d-ba1e-4519-816c-11c525132a3d`. Post-deployment TEST state remains all Candidate feature flags false, Candidate auto-authorisation false, all seven Candidate tables empty and all fourteen Candidate business RPCs installed. The 25-page current-decisions PDF was rendered and rechecked; it requires no amendment because this correction implements the existing whole-record rejection, expense-isolation, immutable-history and PAPER-retirement policy.
