...
For `WORKER_SUBMIT`, `candidate_signed_at_utc` is a required factual input. For exact replay, the key is bound to client semantics while server-generated delivery material is deterministically regenerated or ignored by the semantic hash. Finalisation and PAPER retry use receipt-first execution contracts; the normal Office PAPER retry may return `202 RETRY_IN_PROGRESS` for a concurrent exact request and `200 READY` after the original attempt has completed. No Office response exposes R2 storage identity.
