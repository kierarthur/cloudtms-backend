...
- Public Candidate access and refresh values are independently encrypted broker envelopes bound to audience, environment, frozen private issue/expiry facts and a stable opaque public session identity. New v3 envelopes embed the issuing key version and use a domain-separated HMAC identity plus a distinct per-message AES-GCM key, so an exact durable private replay produces the same public credential strings without using one key/IV for different plaintext. Explicit reader catalogues retain approved v1/v2/v3 versions and can retire a version without accepting it merely because its secret remains bound.
