...
For `WORKER_SUBMIT`, `candidate_signed_at_utc` is a required factual input. For exact replay, the key is bound to factual client semantics while server-generated review presentation is excluded from the semantic hash. A valid expense submission without a target row uses a deterministic provisional identity, never a fresh random UUID. Reminder and renewal requests carry the exact approval-request ID and request generation from the server action envelope. Finalisation and PAPER retry use receipt-first execution contracts; the normal Office PAPER retry may return transient `202 RETRY_IN_PROGRESS`, but after completion the same UUID returns the durable final `READY`, `FAILED_RETRYABLE` or `FAILED_TERMINAL` result and HTTP status. No Office response exposes R2 storage identity.
