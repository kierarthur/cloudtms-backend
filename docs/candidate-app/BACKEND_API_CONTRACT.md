...
- EMAIL manager access is independent of the Candidate session. Same-phone PHONE access is a separate short-lived deterministic-v4 broker envelope. Its factual mutation receipt is bound to the initiating public Candidate session and, where supplied, device identity before token generation; the raw private PHONE token is never returned by the public API and exact replay uses the receipt's frozen private and public handoff-key versions.
...
- All factual mutations require a caller-owned idempotency key; the backend never substitutes a random key. Challenge start/resend rejects a missing or oversized key and preserves `CANDIDATE_IDEMPOTENCY_CONFLICT`; only eligibility/account-state results are enumeration-masked. Resend-too-soon and resend-limit are visible durable 429 results, not accepted 202 responses; exact replay returns the same throttle and a later resend requires a new key. Challenge receipts freeze the issuing token-key version and replay it only through the explicit retained-reader catalogue. Unknown-account login records the generic failed result without creating or mutating an account, so exact replay is stable and changed factual input conflicts. Logout is authenticated and must pass through public-access unwrap before the signed private call. Exact replay is probed before mutable enrichment where the semantic request is already knowable. Session-producing handlers reconstruct the returned refresh token from the database result's session ID and retained key version, never from a losing local proposal. Logout and password change perform a second exact-receipt recovery after a concurrent mutable-precondition failure. Push registration hashes the broker's stable provider/session/token HMAC identity while retaining randomized encrypted token storage; an overlapping proof catalogue lets the private receipt reuse the originally frozen identity version across an approved rotation. Stale workflow generations, route contexts and lifecycle row signatures fail closed.
...
| POST | `/candidate-app/v1/auth/challenge/resend` | Supersede/resend an eligible challenge; 202 means durable acceptance, while timing/allowance throttles are durable public 429 results. |
...
For `WORKER_SUBMIT`, `candidate_signed_at_utc` is a required factual input. For exact replay, the key is bound to factual client semantics while server-generated review presentation is excluded from the semantic hash; the receipt probe occurs before mutable financial/presentation rebuild. A valid expense submission without a target row uses a deterministic provisional identity, never a fresh random UUID. Reminder and renewal requests carry the exact approval-request ID and request generation from the server action envelope, and generated manager delivery values are excluded from semantic request identity. Finalisation completion is bound to the immutable approval/PAPER identity rather than the incidental trigger key. PAPER retry is receipt-first and obeys backoff and lease expiry; the normal Office PAPER retry may return transient `202 RETRY_IN_PROGRESS`, but after completion the same UUID returns the durable final `READY`, `FAILED_RETRYABLE` or `FAILED_TERMINAL` result and HTTP status. No Office response exposes R2 storage identity.
...
QR token creation is not document readiness. The current canonical `TIMESHEET` document version must first be `READY`; the private scheduled worker then assembles one complete immutable pack in exact manifest order: official unsigned timesheet, Mileage Claim Form where applicable, and every evidence page. `EXPENSE_SUMMARY` is prohibited in both promoted legacy and current V2 outgoing/return manifests. The scheduled worker alone releases the exact workflow/generation/manifest-bound mail operation and idempotent Candidate notification. A `FAILED` mail row remains failed until the existing explicit mail-retry authority acts; Candidate polling cannot requeue it. The status and download GETs are strictly read-only. The Worker verifies Candidate ownership and streams bytes from R2; the broker/app never receives an R2 key.

The Expense and Mileage Approval Summary is an internal, automatically refreshed Office evidence document. It is never Candidate-viewed, manager-reviewed, manager-signed or manager-dated, and it is never an outgoing or returned PAPER/QR page. It retains configured agency branding; Candidate, client, contract and week identity; each non-zero category total; and the exact de-duplicated supporting-evidence count for that category. Its generation is content-addressed and stale-render fenced, and zero current expense values remove it. The A4 Mileage Claim Form remains a claim page: it uses configured branding, the title `Mileage Claim Form for week ending dd/mm/yyyy`, repeatable `Post Code from`, `Cost Code To` and `Number of miles` rows, total mileage, manager signature/date and stable workflow/page identity. These documents format frozen canonical claim facts only; no renderer invents a replacement total by summing unrelated fields.
...
The initial manager email is also readiness-gated. Candidate submission first renders and registers every required review component: the candidate-signed hours page where applicable, the mileage form and each required evidence image/PDF. The internal `EXPENSE_SUMMARY` is deliberately absent from this signed decision manifest; the manager still sees current category/amount facts through the immutable claim presentation and supporting pages. The workflow becomes `READY_FOR_MANAGER_APPROVAL` only when the immutable review manifest reports every required component ready. `CREATE_EMAIL_APPROVAL_REQUEST` recomputes and verifies that same manifest before it can queue the email. Consequently, the manager link opens already-created pages; it never asks the manager to wait for the review pack to render. Rendering failure leaves the workflow pending/retryable and prevents the email request.
...
Queue time is never treated as sent time. The exact provider-accepted bound outbox receipt owns initial/latest send timestamps and therefore the 24-hour reminder boundary. `REMIND` requires and preserves the exact pending approval-request ID/generation from the action envelope, rotates that request's token, allows at most five resends and is blocked while an exact delivery remains pending. `RENEW` also requires the exact expired request identity; it creates a new request generation/token/expiry and a fresh resend allowance. Neither action silently substitutes a later request. `CANCEL` requires a non-empty reason of at most 1,000 characters, retires the request delivery authority before lifecycle mutation, records the reason and creates one deterministic withdrawal only where provider acceptance is proved.
...
Every Office surface receives one server-owned Candidate applicability decision. Manual non-QR, HealthRoster-authoritative and NHSP-authoritative rows carry `candidate_office_projection_not_applicable=true`; Summary, Simple Timesheet, Bulk Process and Bulk Authorise render a blank Candidate state and do not call the Candidate projection endpoint for those rows. Applicable Summary rows carry their compact projection in the initial authenticated Summary response, while applicable modal/detail refreshes use the exact Office projection route. Financial completion alone is never converted into Candidate completion; only durable workflow finalisation may display `Candidate Submission Complete`.

Office Processing Status is operational rather than Candidate-lifecycle text. Its closed display catalogue is `Unprocessed`, `Processed`, `Authorised for Invoicing`, `Partially Invoiced`, `Invoiced`, `Archived` and `Processing Delayed`. QR waiting/preparation/return state belongs only to Candidate Submission. Authorise and Unauthorise consume the canonical server action flags and `PENDING_AUTH`/`READY_FOR_HR` authority. The complete signed QR return manifest must first be accepted and canonically finalised before a QR row reaches that authority; issued unsigned material, partial return and legacy QR fields cannot enable Authorise. Deliberate Manual conversion returns the row to the normal Manual lifecycle.
...
Password verification is not authorised by a REST snapshot. For known-account login and authenticated password change, the private backend derives the submitted PBKDF2 digest using the inspected verifier and sends that non-plaintext digest together with a SHA-256 fingerprint of the exact inspected password authority. After receipt ownership and the shared account-session lock, `candidate_auth_account_transition_v1` locks the account row, recomputes the authority fingerprint and compares the presented digest with the locked current verifier. Stale positive or negative login verdicts produce the durable generic invalid result without session/counter mutation; a password change whose proof no longer matches cannot replace the winner or revoke sessions. Raw current/new passwords never enter the RPC payload.
