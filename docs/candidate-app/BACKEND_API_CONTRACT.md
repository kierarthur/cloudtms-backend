...
- All factual mutations require a caller-owned idempotency key; the backend never substitutes a random key. Exact replay is probed before mutable enrichment where the semantic request is already knowable. Stale workflow generations, route contexts and lifecycle row signatures fail closed.
