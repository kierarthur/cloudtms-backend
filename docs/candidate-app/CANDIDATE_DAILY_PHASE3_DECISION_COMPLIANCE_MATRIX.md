...
| AV-298 | The global Candidate key is generated in Google from the normalized Credentially Public ID and is the exact safe value the CloudTMS administrator enters on that Candidate record. | Certified Master source implements `CID1-<Crockford Base32 payload><4-char keyed checksum>`; the raw Public ID is not entered in CloudTMS. The separate runtime source HMAC remains distinct. | PASS IN SOURCE; ADMIN ENTRY/BOOTSTRAP PROOF LATER |

## Pre-existing decisions

All accepted Sections 1–88 and AV-001–AV-249 remain controlling. AV-250–AV-298 are additive Phase 3 decisions and do not activate Candidate Daily or retire any legacy system. AV-271–AV-280 are later-controlling where R11 wording could otherwise permit mirroring deferred/rejected rows or clearing a non-terminal Availability recovery operation. AV-281–AV-298 are later-controlling for Master Rota durability, Google quota ownership, population-wide TEST enablement and the distinct CID1 global-key versus Daily Google source-HMAC authorities; they supersede earlier one-candidate/one-cohort technical-gate wording.
