...
The browser/native client receives only broker-sealed access and refresh envelopes. Those envelopes are audience- and environment-bound and contain the corresponding internal Candidate session material encrypted with different broker secrets. New responses use deterministic authenticated v3 envelopes carrying the issuing key version: a domain-separated HMAC of the canonical plaintext selects a distinct per-message AES-GCM key, so the fixed IV is never used for different plaintext under one key and an exact private receipt produces the same public token. Explicit reader catalogues retain supported random-IV v1 and deterministic v2 envelopes during bounded rollout and may retire a version even while its secret remains bound. Public session UUIDs are stable opaque HMAC-derived identities for the immutable internal session. Before a session or PHONE mutation reaches the private authority, the broker proves that every selected public wrapping secret exists. The broker unwraps a supported version only while constructing a signed service-bound request.
...
Public credential rollout uses v3 envelopes with an embedded issuing version for access, refresh and PHONE handoff. Each authority has a current writer version and an explicit reader catalogue. An approved rollback uses the same version-aware reader build with the previous writer version restored while retaining the newer version in the reader catalogue; rollback to a pre-versioned build is forbidden. Login, activation and refresh store the chosen public access/refresh/session versions in the durable private result so a lost-response retry reissues the byte-identical public result. Refresh retains the original public-session mapping version even when access or refresh writer keys rotate.

Unauthenticated routing is a closed catalogue limited to challenge start/resend/verify, password completion, login and refresh. Public logout is never included: it unwraps the Candidate access credential and forwards the exact private bearer. Challenge start/resend validate the bounded caller key at the broker, pass through idempotency errors/conflicts and enumeration-mask only eligibility/account-state outcomes. PHONE selection binds public session and optional supplied device digests into the private semantic mutation receipt before creating or wrapping any handoff token.
...
The current public-authentication runtime correction is published through backend commit `1df31d2f041bb2e6b9381f39dd97a0f63ae7bcd4`. GitHub workflow `31628119602` proved all 35 Candidate PostgreSQL suites on 17.6 and 18.1, and safe migration workflow `31628119591` installed exact repeatable hashes `ac1f99582254f661fec65d3d69624af0648b0618265cd1b2854c046c550623e7` (authentication) and `0ca52de3d26c069846caa7717a88cc75c6c861bf35e3279a9fd8f4752510bb35` (workflow). The reader-capable runtime was deliberately deployed before the final evidence-only source head: private Candidate version `ab5ea859-f68a-41fd-98a0-4c20ed6dac2e`, normal TEST backend version `463e8856-4e78-493e-8d96-742a69963bc1` and public broker version `0f358ca2-4631-42b8-b18b-820fe82f0e69`. That published runtime is the approved compatible rollback target; the earlier pre-v3 binary is not. TEST and production remain strictly separate; no production resource was accessed or deployed.
