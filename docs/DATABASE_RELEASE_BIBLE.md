...
Use only for the one-time transition of the existing LIVE database when its historical `public.schema_migrations` ledger is valid but its schema is older than the current repository contract. `PLAN` is read-only and requires exactly one historical bootstrap marker, at least one installed repository migration, no unknown or duplicate migration names, no ambiguous public repeatable ledger, no managed database identity, and the protected LIVE target. `APPLY` installs only repository migrations absent from the historical ledger, records each successful legacy migration for resumability, and applies the current repeatable authority. A bounded transition bootstrap first installs the old Banking Pay structural repeatable, the complete signed inventory of temporary trigger/RPC shims required by historical objects omitted from the old ledger, and the exact archive-transition capability relation; application traffic must remain disabled during this one-time replay. The release refuses adoption unless current repeatables have replaced every marked transition shim, every security verifier passes, and the installed contract exactly matches the approved contract. Only after those checks pass does one transaction create the LIVE identity and complete private migration/repeatable/release ledgers. It never runs against TEST, never contacts or mutates Supabase, and never performs a blind baseline.

The whole LEGACY_UPGRADE is fail-closed but is not one transaction. Each completed migration and its historical public ledger row may commit before a later repeatable, verifier, contract check or adoption step stops. After every stop, inspect the fresh Miget target and rerun PLAN; never reuse the original pending list. Before another hosted APPLY, require both a complete data-bearing PostgreSQL 17 rehearsal from the preserved encrypted pre-upgrade checkpoint under a non-superuser provider owner and a fresh schema/ACL rehearsal of the exact interrupted Miget state. Never weaken `FORCE ROW LEVEL SECURITY` to obtain a rehearsal dump.

All routine-security fingerprints must use provider-neutral catalogue identities: routine schema/name plus each input type's schema/name in ordinal order. Never hash `regprocedure` display text because it changes with session `search_path`. The current Candidate-named LEGACY boundary is one exact 85-routine/eight-service-omission profile; both rehearsals must converge to its complete identity/service/ACL hashes before browser execution is removed.
