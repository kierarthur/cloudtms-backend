...
7. Run `npm run db:check`, the database-release tests, all existing repository tests, the exact security verifiers, and a clean `NEW` replay. Do not publish a change that requires manual ledger repair. When reusing a local PostgreSQL 17 proof database that is behind the current source, run `npm run db:contract:seal-local -- --replay-from=<exact 40-character ancestor commit>` instead of applying files by hand. That mode discovers changed migrations and repeatable closures and replays them only in the canonical `inventory()` / `sqlDateKey()` order before sealing; ordinary filename sorting is not a supported proof route.
...
The PostgREST database URI must retain `options=-c%20pg_show_plans.is_enabled%3Doff%20-c%20statement_timeout%3D120s`. Miget preloads `pg_show_plans`; allowing its collector on CloudTMS PostgREST sessions caused repeated memory warnings and severe complex-RPC latency. The connection-level statement limit is required because a function-level setting does not reliably bound the already-running top-level PostgREST statement. PostgreSQL query diagnostics use the repository-controlled `pg_stat_statements` extension migration instead. Prove the collector is off and the statement limit is 120 seconds after every new database, restore, service resize, or PostgREST redeployment.
