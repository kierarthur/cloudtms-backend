...
- derive every LEGACY general/Candidate routine and ACL fingerprint only from a fresh ACL-preserving restore of the exact hosted checkpoint after applying the complete current preload/repeatable order; the sealed general inventories include `_temp_diag_log` and the sealed interrupted Candidate profile includes exactly five previously installed service grants, while a disposable database already changed by a later failed rehearsal must never be used to bless another profile;
- replace immutable Supabase migration `26072026_1219_disable_runtime_plpgsql_check.sql` during LEGACY_UPGRADE with `supabase/release/30082026_0958_legacy_plpgsql_check_database_defaults_replacement.sql`; it applies and catalogue-verifies the same seven runtime defaults on `current_database()` because Miget uses generated database names rather than hard-coded `postgres`;
...
- Candidate-named browser isolation accepts only the sealed clean service-ACL fingerprint or the separately sealed interrupted fingerprint created by five already-installed repository service grants; both preserve their exact service matrix and converge to zero browser execution, while every third ACL shape fails closed;
